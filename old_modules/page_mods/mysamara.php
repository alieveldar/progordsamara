...
$news=DB("SELECT `id`,`picoriginal`,`username` FROM `_widget_insta` WHERE (`stat`=1) ORDER BY `data` DESC LIMIT 9"); $arn=array();
if ($news["total"]>1) { for ($i=0; $i<$news["total"]; $i++): @mysql_data_seek($news["result"], $i); $ar=@mysql_fetch_array($news["result"]); $arn[]=$ar; endfor; }
