...
$file="_index-indexsmp4g"; if (RetCache($file,'cacheblock')=="true") { list($text,$cap)=GetCache($file,0); }else{ list($text,$cap)=NewIndexPage(); SetCache($file,$text); }
...
function LEFT() {
	...
	$q="SELECT `[table]`.`id`, `[table]`.`name`, `[table]`.`tavto`, `[table]`.`lid`, `[table]`.`data`, `[table]`.`comcount`, `[table]`.`pic`, '[link]' as `link` FROM `[table]` WHERE (`[table]`.`stat`='1' && `[table]`.`redak`!=1 && (`[table]`.`promo`!=1 || `[table]`.`spromo`!=1) [used])"; $endq="ORDER BY `data` DESC LIMIT 98"; $data=getNewsFromLentas($q, $endq);
	...
	foreach($list as $ar) {
		...
		$text.="</div>".$C20;
		if ($cnt%4==0) {
			if ($cnt==4) { $text.='<script type="text/javascript" src="//vk.com/js/api/openapi.js?105"></script><div id="vk_groupsVK"></div><script type="text/javascript">VK.Widgets.Group("vk_groupsVK", {mode: 1, width: "200", height: "140", color1: "FFFFFF", color2: "2B587A", color3: "5B7FA6"}, 40786487);</script>'.$C25; }
...
function CENTER() {
	...
	$cnt=1; foreach($list as $ar) {
		...
		if ($cnt==2) {
			$con=DB("SELECT *, 'concurs' as `linked` FROM `concurs_lenta` WHERE (`stat`=1 && `votingend`>'".time()."') ORDER BY `data` DESC LIMIT 1"); if ($con["total"]==1) {
			@mysql_data_seek($con["result"],0); $ac=@mysql_fetch_array($con["result"]); $text.="<div class='RedNews Editors'>";
			if ($ac["pic"]!="") { $text.="<div class='img'><a href='".$ac["linked"]."' $rel><img src='/userfiles/pictavto/".$ac["pic"]."'></a></div><div class='stext'>"; } else { $text.="<div class='ftext'>"; }
			$text.="<a href='".$ac["linked"]."/view/".$ac["id"]."' class='caption'>".$ac["name"]."</a>"; if ($ac["lid"]!="") { $text.="<div class='lid'>".$ac["lid"]."</div>"; } $text.="</div></div>".$C30;
		}}
