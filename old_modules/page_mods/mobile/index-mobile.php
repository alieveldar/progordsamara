...
<?
//$pg=$dir[1]?$dir[1]:1; $file="_index-samaranewsmob_".(int)$pg; $VARS["cachepages"]=0; $Page["Caption"]="Новости Самары"; $CSSmodules["авто включение ленты"]="/modules/lenta/lenta.css";
...
function KazanNews() { $C=""; 
	global $VARS, $GLOBAL, $C10, $C20, $C25, $C, $used, $VAR, $C, $C20, $C10, $C25, $dir; $onpage=43; $pg=$dir[1]?$dir[1]:1; $from=($pg-1)*$onpage; $onblock=3;
	...
	for ($i=0; $i<$data["total"]; $i++) { @mysql_data_seek($data["result"], $i); $ar=@mysql_fetch_array($data["result"]); $d=ToRusData($ar["data"]); $pic="";
		...
		if($data["total"]>($i+1)){ if (($i+1)%$onblock==0) { if ($j==0) { $text.="<div class='Banner' id='Banner-28-1'></div>"; } else { $text.="<div class='Banner' id='Banner-29-".$b."'></div>".$C10; $b++; } $j++;} else { $text.=$C25; }}}
	// строим пагер =================================
	$q="SELECT `[table]`.`id` FROM `[table]` WHERE (`[table]`.`stat`='1')"; $endq="";
	$data=getNewsFromLentas($q, $endq);	$text.=Pager2($pg, $onpage, ceil($data["total"]/$onpage), "samara-news/"."[page]");
	// ==============================================
 	return (array($text, $C));
}
?>