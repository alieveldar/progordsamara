...
if ($start=="view") {
	...
	$data=DB("SELECT `comments`,`alttext`, `promo` FROM `".$table."` WHERE (`id`='".(int)$dir[2]."' ".$where.") LIMIT 1");
	if ($data["total"]==1) {
...
function GetLentaId() {
	...
	*/
	...
	if ($old==1) { $text="<h1>".$cap."</h1>"."<div class='WhiteBlock LentaItem'>".$lid.$pic.$ban1."<div class='ItemText'>".$maintext."</div>".$cards.$ban2.$report.$video.$album.$endtext.$voting.$event.$contacts.$C10.$likes.$mixblock.$ban3."</div>";
	} else { $text="<h1>".$cap."</h1><div class='WhiteBlock LentaItem' style='padding:9px;'>".$pic.$C15.$lid.$ban1."<div class='ItemText'>".$maintext.$cards.$ban."</div>".$ban2.$report.$video.$album.$endtext.$voting.$event.$contacts.$C10.$likes.$mixblock.$dtags.$ban3."</div>";}
