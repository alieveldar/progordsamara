...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
	...
	if (isset($P["savebutton"])) {
		foreach ($P["Inp"] as $key=>$val) {
			$ch=$P["Ina"]["$key"]; $name=$val; $text=$P["PostText"][$key]; 
			$q="UPDATE `_widget_pics` SET `name`='".$name."', `text`='".$text."', `author`='".$ch."' WHERE (`id`='".(int)$key."' && pid='".(int)$id."')"; DB($q);
		}
		DB("INSERT INTO `_lentalog` (`link`, `id`, `uid`, `data`, `ip`, `text`) VALUES ('".$alias."', '".$id."', '".$_SESSION['userid']."', '".time()."', '".$_SERVER['REMOTE_ADDR']."', 'Редактирование фотоотчета (report)')");
	...
	if ($data["total"]>0) {
		...
		for ($i=0; $i<$data["total"]; $i++): @mysql_data_seek($data["result"],$i); $ar=@mysql_fetch_array($data["result"]); if ($ar["stat"]==1) { $chk0="checked"; } else {$chk0="";}
			$img="<img src='/userfiles/picpreview/".$ar["pic"]."' width='150' />"; $chb=$ar["sets"]==1?"checked":"";
			
			$AdminText.='<tr class="TRLine" id="Line'.$ar["id"].'"><td><table><tr>
				<td style="width:10%;" valign="top" align="center">'.$img.'</td>
				<td class="LongInput" style="width:80%;" valign="top">
					<span style="display:block; float:left; margin-right:5px;"><input type="checkbox" id="RS-'.$ar["id"].'-_widget_pics" value="1" '.$chk0.' /></span><span style="line-height:22px; display:block; float:left;">Отображать фотографию</span>'.$C10.'
					<input name="Inp['.$ar["id"].']" value=\''.$ar["name"].'\' placeholder="Название фотографии">'.$C10.'
					<input name="Ina['.$ar["id"].']" value=\''.$ar["author"].'\' placeholder="Маленькое описание фотографии или имя автора (255 символов)">
				</td></tr>
				'."<tr style='border-bottom:2px dotted #CCC;'><td colspan='2' style='padding-bottom:15px;'><textarea name='PostText[".$ar["id"]."]' id='textedit".$ar["id"]."' style='outline:none;' class='texteditors'>".$ar["text"]."</textarea></td></tr>".'
				</table>
			</td>
	...
	<div class='SecondMenu2'><a href='?cat=".$alias."_report&id=".$id."'>Виджет: Фото-отчет</a></div>
