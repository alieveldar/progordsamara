...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
	...
	if (isset($P["savebutton"])) {	
		...
		if ($P["vtext"]!="") {
			...
			if ($data["total"]==0) { DB("INSERT INTO `$table2` (`pid`, `link`, `name`, `text`) VALUES (".(int)$id.", '".$alias."', '".$P["vname"]."', '".str_replace("'","\'",$P["vtext"])."')");
			} else { DB("UPDATE `$table2` SET `name`='".$P["vname"]."', `text`='".str_replace("'","\'",$P["vtext"])."' WHERE (`pid`=".(int)$id." AND `link`='".$alias."') LIMIT 1 "); }
