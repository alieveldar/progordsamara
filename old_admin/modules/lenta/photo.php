...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
	...
	if (isset($P["savebutton"])) {
		if ($P["delpic"]==1) {
			$q="UPDATE `".$alias."_lenta` SET `picxy`='', `picauth`='', `pic`='' WHERE (id='".(int)$id."')"; DB($q);
			$msg="Фотография удалена";
		} else {
	...
	if ($node["pic"]!="") { 
		...
		$AdminText.=$C10."<div class='Info'><input type='checkbox' name='delpic' value='1'>  удалить фотографию (нельзя отменить)</div>";
