...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
	...
	if (isset($P["savebutton"])) {
		foreach ($P["Inp"] as $key=>$val) { 
			$name=$val; $text=$P["PostText"][$key]; $q="UPDATE `_widget_pics` SET `name`='".$name."' WHERE (`id`='".(int)$key."' && pid='".(int)$id."')"; DB($q);
		}
	...
	@mysql_data_seek($data["result"], 0); $node=@mysql_fetch_array($data["result"]); if ($node["stat"]==1) { $chk="checked"; } else {$chk="";}
