...
var button='<div class="CenterText" id="JSLoader"><input type="submit" name="addbutton" class="SaveButton" value="Добавить" onclick="ldr"></div><input type="hidden" name="pid" value="'+pid+'"/>';
...
var button='<div class="CenterText" id="JSLoader"><input type="submit" name="savebutton" class="SaveButton" value="Сохранить" onclick="ldr"></div><input type="hidden" name="id" value="'+id+'"/><input type="hidden" name="pic" value="'+pic+'"/>';
...
function ActionAndUpdate(id, act, tab, pid) {CloseBlank(); JsHttpRequest.query('modules/adm/placescats-JSReq.php',{'id':id,'act':act,'tab':tab,'pid':pid},function(result,errors){ if(result){ $("#Tgg").html(result["content"]); $("#Msg2").html("Данные успешно сохранены"); $("#Msg2").removeClass(); $("#Msg2").addClass("SuccessDiv"); $('#Tgg input').tzCheckbox({labels:['да ','нет']}); }},true); } function ReturnI(id, tab) { $("#Act"+id).html('<a href="javascript:void(0);" onclick="ItemDelete(\''+id+'\',\''+tab+'\')"><img src="/admin/images/icons/exit.png" valign="middle" title="" style="margin:-2px 3px 0 0;"></a>'); }

function ldr() { $("#JSLoader").html("/admin/images/loader2.gif"); }
