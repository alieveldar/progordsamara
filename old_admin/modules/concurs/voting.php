...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
	...
	if (isset($P["savebutton"])) {
		...
		$q="UPDATE `".$alias."_lenta` SET `voting`=".$P["voting"].",`elemsstyle`=".$P["elemsstyle"].",`votingend`='".$sdata1."',`winnerscount`=".$P["winnerscount"]." WHERE (id='".(int)$id."')"; DB($q); 
		
		foreach ($P["Inp"] as $key=>$val) {	$name=$val;
		$q="UPDATE `_widget_pics` SET `name`='".str_replace("'","\'",$name)."',`text`='".str_replace("'","\'",$P["PostText"][$key])."' WHERE (`id`='".(int)$key."' && pid='".(int)$id."')"; DB($q); }
	...
	if ($node["elemsstyle"]==2) { $vc1="selected"; } elseif ($node["elemsstyle"]==1) { $vc2="selected"; } 
	...
	$AdminText.='<tr class="TRLine1"><td class="VarText">Отображение элементов</td><td class="LongInput"><div class="sdiv"><select name="elemsstyle"><option value="2" '.$vc1.'>Изображение с описанием</option><option value="1" '.$vc2.'>Только изображение</option></select></div></td><tr>';
	...
	if($GLOBAL["now"] > $node["votingend"]) {
		...
		if($data['total']){
			...
			for ($i=0, $j=0; $i<$data["total"]; $i++){
				...
				$items.='<li>';
				$items.='<div class="votingImg"><a href="javascript:void(0);" title=\''.$ar["name"].'\' onclick=\'ViewBlank("", "<img src=/userfiles/picoriginal/'.$ar["pic"].' />");\'><img title=\''.$ar["name"].'\'  src="/userfiles/picpreview/'.$ar['pic'].'" border="0" /></a></div>';				
				$items.='<div class="votingText"><h3 class="votingName">'.$ar["name"].'</h3>'.$ar["text"].'</div>';
			...
			$AdminText.='<ul class="voting elemsStyle0">'.$items.'</ul>'.$C;
		}
	}
	...
	if ($data["total"]>0) {
		...
		for ($i=0; $i<$data["total"]; $i++): @mysql_data_seek($data["result"],$i); $ar=@mysql_fetch_array($data["result"]); if ($ar["stat"]==1) { $chk0="checked"; }
			...
			$AdminText.="<textarea name='PostText[".$ar["id"]."]' id='textedit".$ar["id"]."' style='outline:none;' class='texteditors' placeholder='Описание'>".$ar["text"]."</textarea>";
