...
if ($GLOBAL["sitekey"]!=1) { $ROOT = $_SERVER['DOCUMENT_ROOT']; $GLOBAL["sitekey"] = 1; $now=time();
require_once($ROOT."/modules/standart/DataBase.php");	 } require_once("pclzip.lib.php"); $text="";
// === === === === === === === === === === === === === === === === === === === === === === === === === === === === === ===

$lentas=array("news"=>"Новости Самары","auto"=>"Дороги","sport"=>"Спорт","business"=>"Бизнес");

// === === === === === === === === === === === === === === === === === === === === === === === === === === === === === ===
### ПОГОДА
	$xml=simplexml_load_file("http://export.yandex.ru/bar/reginfo.xml?region=51"); $lvl=$xml->traffic->level; $icnt=$xml->traffic->icon; $txtt=$xml->traffic->hint; $temp=$xml->weather->day->day_part->temperature;
	...
	$text.="<weather><item><text><![CDATA[ Погода в Самаре: <b>".$temp."</b>°C     Пробки: <b>".$lvl."</b>     <b>$</b>=<b>".round($usd,2)."</b> руб.     <b>€</b>=<b>".round($euro,2)."</b> руб. ]]></text></item></weather>\r\n";
	
	
### МЕНЮ
$text.="<menu>"; foreach ($lentas as $t=>$n) { $text.="<item><title>".$n."</title><link>newslist</link><id>".$t."</id></item>";  } $text.="</menu>"."\r\n";
...
if ($ap["name"]!="") { $ar["text"].="<h3>".$ap["name"]."</h3>"; } $ar["text"].="<img src=\"http://progorodsamara.ru/userfiles/picintv/".$ap["pic"]."\">"; if ($ap["text"]!="") { $ar["text"].=$ap["text"]; } endfor; }
...
<avatar>http://progorodsamara.ru/".$ar["avatar"]."</avatar>
<data>$ar[data]</data>
<rdata>$rdata</rdata>
<link>".$ar["link"]."</link>
...
// === === === === === === === === === === === === === === === === === === === === === === === === === === === === === === 
function nlbr($text) { $text=str_replace(array("\t","\r","\n","&nbsp;", 'src="/userfiles'), array("","",""," ", 'src="http://progorodsamara.ru/userfiles'), $text); /*$text=htmlspecialchars($text, ENT_QUOTES);*/ return $text; } function GetMobileVideo($text) { $text=preg_replace('~width="\d+"~', 'width="320"', $text); $text=preg_replace('~height="\d+"~', 'height="300"', $text);  $text=str_replace("<iframe ", '<iframe allowCrossDomainXHR="true" sandboxRoot="http://www.youtube.com/" documentRoot="app:/sandbox/" ', $text); return $text; } function ToRusData($var) { $var = date("Y.m.d.H.i.s", $var); list($y, $m, $d, $h, $i, $s)=explode(".", $var); $data=$h.":".$i.", ".$d.".".$m.".".$y; return($data); }
?>