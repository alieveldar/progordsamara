...
$q=""; foreach($tables as $table) { $tmp=explode("_", $table); $link=$tmp[0]; $q.="(SELECT `$table`.`id`, `$table`.`name`, `$table`.`lid` as `text`, `$table`.`data`, `$table`.`pic`, `_pages`.`link` FROM `$table` LEFT JOIN `_pages` ON `_pages`.`link`='$link'  WHERE (`$table`.`stat`='1' && `$table`.`spec`='1' && `$table`.`data`>'".(time()-3*24*60*60)."') GROUP BY 1) UNION ";}
$qq=trim($q, "UNION ")." ORDER BY `data` DESC LIMIT 5"; $datat=DB($qq); echo $qq; 
