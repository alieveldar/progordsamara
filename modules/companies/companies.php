...
$table = $link."_cats";
$table2 = $link."_items";
$table3 = $link."_contacts";
$table4 = $link."_actions";
$table5 = $link."_qa";
$table6 = "_widget_pics";
if ($start == "") {
    $start = "cats";
...
if ($start == "view") {
    ...
    $data = DB(
      "SELECT `".$table2."`.* FROM `".$table2."` WHERE (`$table2`.`id`='".(int)$dir[2]."' $where) GROUP BY 1 LIMIT 1"
    );
    ...
    if ($data["total"]) {
        ...
        if (RetCache($file) == "true") {
            ...
            list($text, $cap) = GetCompanyId($item);
...
if ($start == 'cat') {
    ...
    if (RetCache($file) == "true") {
        ...
        list($text, $cap) = GetCompaniesList();
        SetCache($file, $text, $cap);
    }
}
...
if ($start == 'question' && $dir[2] == 'view') {
    ...
    $data = DB(
      "SELECT `".$table5."`.*, `".$table."`.id AS cid, `".$table."`.name AS cname FROM `".$table5."` LEFT JOIN `".$table."` ON `".$table."`.id=`".$table5."`.rid WHERE (`$table5`.`id`='".(int)$dir[3]."') GROUP BY 1 LIMIT 1"
    );
    ...
    if ($data["total"]) {
        ...
        if (RetCache($file) == "true") {
            ...
            list($text, $cap) = GetQuestionId($item);
            SetCache($file, $text, $cap);
        }
        UserTracker($link, $page);
    } else {
...
if ($start == 'answer' && $dir[2] == 'add') {
    $data = DB(
      "SELECT `".$table5."`.*, `".$table."`.name AS catname FROM `".$table5."` LEFT JOIN `".$table."` ON `".$table."`.id=`".$table5."`.rid WHERE (`$table5`.`id`='".(int)$dir[3]."') GROUP BY 1 LIMIT 1"
    );
    if ($data['total']) {
        @mysql_data_seek($data["result"], 0);
        $item = @mysql_fetch_array($data["result"]);
        list($text, $cap) = addAnswer($item);
        $cap = 'Добавление ответа';
    } else {
        $text = @file_get_contents($ROOT."/template/404.html");
        $cap = "Материал не найден";
        $Page404 = 1;
...
if ($start == 'answer' && $dir[2] == 'edit') {
    $data = DB(
      "SELECT `t1`.*, `t2`.`name` AS `pname`, `t2`.`text` AS `ptext`, `".$table."`.name AS catname FROM `".$table5."` AS `t1` LEFT JOIN `".$table5."` AS `t2` ON `t2`.`id`=`t1`.`pid` LEFT JOIN `".$table."` ON `".$table."`.id=`t1`.rid WHERE (`t1`.`id`='".(int)$dir[3]."') GROUP BY 1 LIMIT 1"
    );
    if ($data['total']) {
        @mysql_data_seek($data["result"], 0);
        $item = @mysql_fetch_array($data["result"]);
        list($text, $cap) = editAnswer($item);
        $cap = 'Редактирование ответа';
    } else {
        $text = @file_get_contents($ROOT."/template/404.html");
        $cap = "Материал не найден";
        $Page404 = 1;
...
if ($start == 'actions' && (int)$dir[2]) {
    ...
    if (RetCache($file) == "true") {
        ...
        list($text, $cap) = GetCompanyActionsList();
        SetCache($file, $text, $cap);
    }
}
...
if ($start == 'action' && $dir[2] == 'add') {
    $where = $GLOBAL["USER"]["role"] == 0 ? "AND `$table2`.`stat`=1" : "";
    $data = DB(
      "SELECT `".$table2."`.id FROM `".$table2."` WHERE (`$table2`.`id`='".(int)$dir[3]."' AND `$table2`.`uid`='".$_SESSION["userid"]."' $where) GROUP BY 1 LIMIT 1"
    );
    if ($data['total']) {
        $text = addAction().getActionForm();
        $cap = 'Добавление акции';
    } else {
        $text = @file_get_contents($ROOT."/template/404.html");
        $cap = "Материал не найден";
        $Page404 = 1;
...
if ($start == 'action' && $dir[2] == 'edit') {
    $data = DB(
      "SELECT `".$table4."`.*, `".$table2."`.uid FROM `".$table4."` JOIN `".$table2."` ON `".$table2."`.id=`".$table4."`.pid AND `$table2`.`uid`='".$_SESSION["userid"]."' WHERE (`$table4`.`id`='".(int)$dir[3]."') GROUP BY 1 LIMIT 1"
    );
    if ($data['total']) {
        @mysql_data_seek($data["result"], 0);
        $action = @mysql_fetch_array($data["result"]);
        $text = saveAction($action).getActionForm($action);
        $cap = 'Редактирование акции';
    } else {
        $text = @file_get_contents($ROOT."/template/404.html");
        $cap = "Материал не найден";
        $Page404 = 1;
...
if ($start == 'action' && $dir[2] == 'view') {
    ...
    $data = DB(
      "SELECT `".$table4."`.* FROM `".$table4."` WHERE (`$table4`.`id`='".(int)$dir[3]."' $where) GROUP BY 1 LIMIT 1"
    );
    ...
    if ($data["total"]) {
        ...
        if (RetCache($file) == "true") {
            ...
            list($text, $cap) = GetActionId($item);
            SetCache($file, $text, $cap);
        }
        UserTracker($link, $page);
...
function GetCompanyId($item)
{
    ...
    if ($item['pic']) {
        $text .= '<div class="nodePic"><a href="/userfiles/picoriginal/'.$item['pic'].'" rel="prettyPhoto[gallery]"><img src="/userfiles/picpreview/'.$item['pic'].'" border="0" /></a></div>';
    }
    $text .= $item['text'];
    $path = '/userfiles/picnews/'.$item["pic"];
    $text .= $C10."<div class='Likes'>".Likes(
        Hsc($cap),
        "",
        "http://".$RealHost.$path,
        Hsc(strip_tags($item['lid']))
      ).$C."</div>";
    if ($item['urik']) {
        $text .= '<div class="urik">'.$item['urik'].'</div>';
    }
    $text .= '</div>';

    $data3 = DB(
      "SELECT `".$table4."`.* FROM `".$table4."` WHERE (`$table4`.`pid`='".$item["id"]."') GROUP BY 1 LIMIT 3"
    );
    if ($data3["total"]) {
        $text .= $C5."<h2>Акции и скидки компании</h2><div class='Actions'>";
        for ($i = 0; $i < $data3["total"]; $i++) {
            @mysql_data_seek($data3["result"], $i);
            $ar3 = @mysql_fetch_array($data3["result"]);
            $pic = '';
            if ($ar3["pic"] != "") {
                $pic = "<a href='/".$dir[0]."/action/view/".$ar3["id"]."'><img src='/userfiles/picnews/".$ar3["pic"]."' title='".$ar3["name"]."' /></a>";
            }
            ...
            if ($ar3["todata"]) {
                $d = ToRusData($ar3["todata"]);
                $text .= "<p>Акция действует до ".$d[2]."</p>";
            }
            $text .= "</div>".$C."</div>";
        }
        $text .= "</div>";
    ...
    $p = DB(
      "SELECT * FROM `".$table6."` WHERE (`pid`='".$item["id"]."' && `link`='".$dir[0]."' && `point`='pics' && `stat`=1) order by `rate` ASC"
    );
    if ($p["total"]) {
        mysql_data_seek($p["result"], 0);
        $ar = @mysql_fetch_array($p["result"]);
        if (!$ar["sets"]) {
            $photos = '<div class="ItemAlbum">';
            for ($i = 0; $i < $p["total"]; $i++): mysql_data_seek(
              $p["result"],
              $i
            );
                $ar = @mysql_fetch_array($p["result"]);
                $photos .= '<a rel="prettyPhoto[gallery]" href="/userfiles/picoriginal/'.$ar["pic"].'"><img src="/userfiles/picnews/'.$ar["pic"].'"></a>'; endfor;
            $photos .= '</div>'.$C;
        } else {
            $photos = '<div class="ItemAlbum2">';
            for ($i = 0; $i < $p["total"]; $i++): mysql_data_seek(
              $p["result"],
              $i
            );
    ...
    if ($data2["total"]) {
        ...
        for ($j = 0; $j < $data2["total"]; $j++) {
            @mysql_data_seek($data2["result"], $j);
            $ar2 = @mysql_fetch_array($data2["result"]);
            $worktimeArr = explode('|', $ar2["worktime"]);
            $worktime = '';
            $workt = false;
            ...
            if ($ar2["adres"]) {
                $text .= "<div><img src='/template/standart/address.png' style='vertical-align:middle;' /><strong class='address'>".$ar2["adres"]."</strong></div>";
            }
            if ($ar2["phone"]) {
                $text .= "<div><img src='/template/standart/phone.png' style='vertical-align:middle;' /><strong class='phone'>".$ar2["phone"]."</strong></div>";
            }
            if ($workt) {
            ...
            if ($ar2["maps"]) {
                $text .= "<span class='maps' style='display:none;'>".$ar2["maps"]."</span>";
            }
            $text .= "<span class='worktime' style='display:none;'>".$ar2["worktime"]."</span></div>";
        }
    }
    if ($item['site']) {
        $text .= '<img src="/template/standart/site.png" style="vertical-align:middle;" /><strong class="address">Сайт:</strong> <a href="'.$item['site'].'" target="_blank"><u>'.$item['site'].'</u></a>';
    }
...
{
    global $VARS, $GLOBAL, $dir, $RealHost, $Page, $node, $table, $table2, $table3, $link, $C, $C5, $C15, $C20, $C10, $C25;
    $d = ToRusData($item["todata"]);

    $data2 = DB(
      "SELECT `".$table2."`.* FROM `".$table2."` WHERE (`$table2`.`id`='".$item["pid"]."') GROUP BY 1 LIMIT 1"
    );
    @mysql_data_seek($data2["result"], 0);
    $comp = @mysql_fetch_array($data2["result"]);
    ...
    if ($data3["total"]) {
        for ($j = 0; $j < $data3["total"]; $j++) {
            @mysql_data_seek($data3["result"], $j);
            $ar3 = @mysql_fetch_array($data3["result"]);
            ...
            $ar3["phone"] = preg_replace('/(\)\s*)+/', ') ', $ar3["phone"]);
            if ($ar3["phone"]) {
                $text .= "<strong class='phone'>тел: <span>".$ar3["phone"]."</span></strong>";
            }
        }
    }
    $text .= '</div>'.$C15;
    ...
    if ($item['rest']) {
        $text .= $C10.'<div class="Info">'.$item['rest'].'</div>';
    }
    if ($item['pics']) {
    ...
    if ($comp['pic']) {
        $text .= "<div class='companyPic'><a href='/".$dir[0]."/view/".$comp["id"]."'><img src='/userfiles/picpreview/".$comp["pic"]."' title='".$comp["name"]."' /></a></div>";
    }
    $text .= "<div class='companyContent'><h2><a href='/".$dir[0]."/view/".$comp["id"]."' class='CompanyName'>".$comp["name"]."</a></h2>";
    if ($comp["anonce"] != "") {
        $text .= $C5."<p>".$comp["anonce"]."</p>";
    }
    $text .= "</div>";
    ...
    if ($data3["total"]) {
        ...
        for ($j = 0; $j < $data3["total"]; $j++) {
            @mysql_data_seek($data3["result"], $j);
            $ar3 = @mysql_fetch_array($data3["result"]);
            $worktimeArr = explode('|', $ar3["worktime"]);
            $worktime = '';
            $workt = false;
            for ($i = 0; $i < sizeof($worktimeArr); $i++) {
                $worktime .= '<td>'.$worktimeArr[$i].'</td>';
            ...
            $text .= "<div class='contacts'>";
            if ($ar3["adres"]) {
                $text .= "<div><img src='/template/standart/address.png' style='vertical-align:middle;' /><strong class='address'>".$ar3["adres"]."</strong></div>";
            }
            if ($ar3["phone"]) {
                $text .= "<div><img src='/template/standart/phone.png' style='vertical-align:middle;' /><strong class='phone'>".$ar3["phone"]."</strong></div>";
            }
            if ($workt) {
                $worktime = '<table class="worktimeTable"><tr><th>Понедельник</th><th>Вторник</th><th>Среда</th><th>Четверг</th><th>Пятница</th><th>Суббота</th><th>Воскресенье</th></tr><tr>'.$worktime.'</tr></table>';
                $text .= $worktime.'<br>';
            ...
            if ($ar3["maps"]) {
                $text .= "<span class='maps' style='display:none;'>".$ar3["maps"]."</span>";
            }
            $text .= "<span class='worktime' style='display:none;'>".$ar3["worktime"]."</span></div>";
        }
    }
    if ($comp['site']) {
        $text .= '<img src="/template/standart/site.png" style="vertical-align:middle;" /><strong class="address">Сайт:</strong> <a href="'.$comp['site'].'" target="_blank"><u>'.$comp['site'].'</u></a>';
    }
    if ($comp['urik']) {
        $text .= '<div class="urik">'.$comp['urik'].'</div>';
    }
    $text .= $C.'<script type="text/javascript">initMap('.$comp["id"].');</script></div></div>';
    if ($item['fas']) {
        $text .= $C10.'<div class="fas">'.$item['fas'].'</div>';
    }
...
function GetConsultId($item)
{
    ...
    $onpage = $node["onpage"];
    $pg = $dir[3] ? $dir[3] : 1;
    $orderby = "ORDER BY `".$table2."`.`vip` DESC";
    $from = ($pg - 1) * $onpage;
    $onblock = 4; /* Новостей в каждом блоке */
    $data = DB(
      "SELECT `".$table2."`.id, `".$table2."`.name, `".$table2."`.pic, `".$table2."`.anonce, `".$table2."`.vip, `".$table."`.name as `cname` FROM `".$table2."` LEFT JOIN `".$table."` ON `".$table."`.`id`=".$dir[2]." WHERE (`".$table2."`.`consultscats` LIKE '%,".$dir[2].",%' $where) GROUP BY 1 ".$orderby." LIMIT $from, $onpage"
    );
    if ($data["total"]) {
        ...
        for ($i = 0; $i < $data["total"]; $i++) {
            ...
            $pic = '';
            if ($ar["pic"] != "") {
                $pic = "<a href='/".$dir[0]."/action/view/".$ar["id"]."'><img src='/userfiles/picpreview/".$ar["pic"]."' title='".$ar["name"]."' /></a>";
            }
            if ($ar["vip"]) {
                $text .= "<div class='WhiteBlock Vip Action'>";
            } else {
                $text .= "<div class='WhiteBlock Action'>";
            }
            $text .= "<div class='companyPic'>$pic</div>";
            $text .= "<div class='actionContent'><a href='/".$dir[0]."/view/".$ar["id"]."'>".$ar["name"]."</a>";

            $data2 = DB(
              "SELECT * FROM ".$table3." WHERE (`pid`=".$ar["id"].")"
            );
            if ($data2["total"]) {
                for ($j = 0; $j < $data2["total"]; $j++) {
                    @mysql_data_seek($data2["result"], $j);
                    $ar2 = @mysql_fetch_array($data2["result"]);
                    $ar2["phone"] = preg_replace(
                      '/(\s*\()+/',
                      ' (',
                      $ar2["phone"]
                    );
                    ...
                    if ($ar2["adres"]) {
                        $text .= "<strong class='address'>".$ar2["adres"]."</strong>";
                    }
                    if ($ar2["adres"] && $ar2["phone"]) {
                        $text .= "<strong class='address'>. </strong>";
                    }
                    if ($ar2["phone"]) {
                        $text .= "<strong class='phone'>тел: <span>".$ar2["phone"]."</span></strong>";
                    }
    ...
    if ($UserSetsSite[5] == '1') {
        $text .= '<tr class="TRLine0"><td class="VarText">Код с картинки<star>*</star></td><td><img src="/modules/standart/captcha/Captcha.php?'.time(
          ).'" class="captchaImg" /><input name="captcha" type="text" class="JsVerify2 captchaInput"><input type="submit" name="QuestionButton" class="SaveButton" style="margin-left:182px;" value="Отправить"></td></tr>';
    }
    $text .= '</table>';
    $text .= '</form></div>';

    $data3 = DB(
      "SELECT t1.`id`, t1.`name`, t1.`text`, COUNT(t2.`id`) AS `cnt` FROM ".$table5." AS t1 JOIN ".$table5." AS t2 ON t2.`pid`=t1.`id` WHERE (t1.`pid`=0 AND t1.rid=".$dir[2].") GROUP BY 1"
    );
    if ($data3["total"]) {
...
{
    global $VARS, $GLOBAL, $dir, $RealHost, $Page, $node, $table, $table2, $table3, $table5, $link, $C, $C5, $C15, $C20, $C10, $C25;
    $d = ToRusData($item["todata"]);

    $data2 = DB(
      "SELECT `".$table5."`.*, `".$table2."`.name AS cname FROM `".$table5."` LEFT JOIN `".$table2."` ON `".$table2."`.id=`".$table5."`.cid WHERE (`$table5`.`pid`='".$item["id"]."') GROUP BY 1"
    );
    ...
    if ($data2["total"]) {
        for ($i = 0; $i < $data2["total"]; $i++) {
            @mysql_data_seek($data2["result"], $i);
            $ar2 = @mysql_fetch_array($data2["result"]);
            $text .= $C10.'<div class="CBG"></div><a name="answer'.$ar2["id"].'"></a>'.$C10;
            if ($ar2["pic"]) {
                $text .= '<div class="answerPic"><img src="/userfiles/avatar/'.$ar2["pic"].'" title="'.$ar2["name"].'" /></div>';
            }
...
function GetCompaniesList()
{
    ...
    $onblock = 4; /* Новостей в каждом блоке */
    $data = DB(
      "SELECT `".$table2."`.id, `".$table2."`.name, `".$table2."`.pic, `".$table2."`.anonce, `".$table2."`.vip, `".$table."`.name as `cname` FROM `".$table2."` LEFT JOIN `".$table."` ON `".$table."`.`id`=".$dir[2]." WHERE (`".$table2."`.`cats` LIKE '%,".$dir[2].",%' $where) GROUP BY 1 ".$orderby." LIMIT $from, $onpage"
    );
    if ($data["total"]) {
        ...
        for ($i = 0; $i < $data["total"]; $i++) {
            ...
            $pic = '';
            if ($ar["pic"] != "") {
                $pic = "<a href='/".$dir[0]."/view/".$ar["id"]."'><img src='/userfiles/picpreview/".$ar["pic"]."' title='".$ar["name"]."' /></a>";
            }
            if ($ar["vip"]) {
                $text .= "<div class='WhiteBlock Vip' id='company_".$ar["id"]."'>";
            } else {
            ...
            $text .= "</div>";
            $data3 = DB(
              "SELECT `".$table4."`.* FROM `".$table4."` WHERE (`$table4`.`pid`='".$ar["id"]."') GROUP BY 1 LIMIT 3"
            );
            if ($data3["total"]) {
    ...
    $data = DB(
      "SELECT count(id) as `cnt` FROM `".$table2."` WHERE (`".$table2."`.`cats` LIKE '%,".$dir[2].",%' $where)"
    );
    $text .= Pager2(
      $pg,
      $onpage,
...
function GetCompanyActionsList()
{
    ...
    $where = $GLOBAL["USER"]["role"] == 0 ? "AND `".$table4."`.`stat`=1" : "";
    $data = DB(
      "SELECT `".$table4."`.*, `".$table2."`.id as `cid`, `".$table2."`.name as `cname` FROM `".$table4."` JOIN `".$table2."` ON `".$table2."`.id=`".$table4."`.pid AND `$table2`.`stat`=1 WHERE (`$table4`.`pid`='".(int)$dir[2]."' $where) GROUP BY 1"
    );
    if ($data["total"]) {
        $text .= "<div class='Actions'>";
        for ($i = 0; $i < $data["total"]; $i++) {
            ...
            $pic = '';
            if ($ar["pic"] != "") {
                $pic = "<a href='/".$dir[0]."/action/view/".$ar["id"]."'><img src='/userfiles/picnews/".$ar["pic"]."' title='".$ar["name"]."' /></a>";
            }
...
function GetActionsList()
{
    ...
    $where = $GLOBAL["USER"]["role"] == 0 ? "AND `".$table2."`.`stat`=1" : "";
    $data = DB(
      "SELECT `".$table2."`.id, `".$table2."`.name, `".$table2."`.vip, `".$table4."`.id AS `aid`, `".$table4."`.name AS `aname`, `".$table4."`.pic AS `apic` FROM `".$table2."` LEFT JOIN `".$table4."` ON `".$table4."`.pid=`".$table2."`.id AND `".$table4."`.stat=1 WHERE (`$table2`.`stat`=1) ORDER BY `$table2`.`vip` DESC"
    );
    if ($data["total"]) {
        ...
        for ($i = 0; $i < $data["total"]; $i++) {
            ...
            if ($ar["apic"] != "") {
                $pic = "<a href='/".$dir[0]."/action/view/".$ar["aid"]."'><img src='/userfiles/picnews/".$ar["apic"]."' title='".$ar["aname"]."' /></a>";
            }
...
function GetActionsRList()
{
    ...
    $where = $GLOBAL["USER"]["role"] == 0 ? "AND `".$table2."`.`stat`=1" : "";
    $data = DB(
      "SELECT `".$table2."`.id, `".$table2."`.name, `".$table2."`.vip, `".$table."`.name AS `cname`, `".$table4."`.id AS `aid`, `".$table4."`.name AS `aname`, `".$table4."`.pic AS `apic` FROM `".$table."` JOIN `".$table2."` ON `".$table2."`.`cats` LIKE '%,".$dir[2].",%' LEFT JOIN `".$table4."` ON `".$table4."`.pid=`".$table2."`.id AND `".$table4."`.stat=1 WHERE (`".$table."`.`id`=".$dir[2].") ORDER BY `$table2`.`vip` DESC"
    );
    if ($data["total"]) {
        $text .= "<div class='ActionsWithCatalog'>";
        for ($i = 0; $i < $data["total"]; $i++) {
...
function GetCats()
{
    ...
    $prlvl = 0;
    $data = DB(
      "SELECT ".$table.".*, COUNT(`".$table2."`.id) AS `cnt` FROM `".$table."` LEFT JOIN `".$table2."` ON `".$table2."`.`cats` LIKE CONCAT('%',',',".$table.".id,',','%') WHERE (".$table.".`stat`=1 && ".$table.".`type`=1) GROUP BY 1 ORDER BY ".$table.".`name`"
    );
    $items[0]["id"] = 0;
    $items[0]["pid"] = 0;
    for ($i = 1; $i <= $data["total"]; $i++): @mysql_data_seek(
      $data["result"],
      ($i - 1)
    );
        $ar = @mysql_fetch_array($data["result"]);
        $idr = $ar["id"];
        $items[$idr]["items"] = $ar["cnt"];
        $items[$idr]["id"] = $ar["id"];
        $items[$idr]["pid"] = $ar["pid"];
        $items[$idr]["name"] = $ar["name"];
        $items[$idr]["text"] = $ar["text"];
        $items[$idr]["pic"] = $ar["pic"];
        $items[$idr]["stat"] = $ar["stat"];
        $items[$idr]["link"] = "/".$link."/cat/".$ar["id"]."/"; endfor;
    $stotal = $data["total"] + 1;
    GetChild_(0);
    $text .= "<ul class='Catalog'>".$itext."</ul>";
...
{
    global $itext, $items, $mvl;
    if ($i != 0) {
        $itext .= HtmlChild_($lvl, $i);
    }
...
{
    global $items, $prid, $prpid, $prlvl, $ul, $r, $am;
    if (!$lvl || !$items[$idi]["items"]) {
        return;
    }
    $pid = $items[$idi]["pid"];
    if ($prlvl > $lvl) {
        for ($k = 0; $k < ($prlvl - $lvl); $k++) {
            $text .= "</ul></li>";
        }
    }
    $text .= "<li class='CatalogLvl".$lvl."'><a href='".$items[$idi]["link"]."' title='".trim(
        $items[$idi]["name"]
      )."'><span class='catImg'><img src='/userfiles/picoriginal/".$items[$idi]["pic"]."' /></span>".trim(
        $items[$idi]["name"]
      )."</a>";
    if (HaveChild_($idi) == 1) {
        $text .= $r."<ul>".$r;
...
function GetChild_C($i, $lvl = -1)
{
    global $itext, $items, $mvl;
    if ($i != 0) {
        $itext .= HtmlChild_C($lvl, $i);
    }
    foreach ($items as $key => $item) {
        if ($item["pid"] == $items[$i]["id"]) {
            $pid = $item["pid"];
...
function HtmlChild_C($lvl, $idi)
{
    ...
    if (!$lvl) {
        $text .= "<li class='WhiteBlock CatalogLvl".$lvl."'><span class='catImg'><img src='/userfiles/picoriginal/".$items[$idi]["pic"]."' /></span><h2>".trim(
            $items[$idi]["name"]
          )."</h2>";
    } else {
        $text .= "<li class='CatalogLvl".$lvl."'><a href='".$items[$idi]["link"]."' title='".trim(
            $items[$idi]["name"]
          )."'>".trim($items[$idi]["name"])."</a>";
    }
    if (HaveChild_C($idi) == 1) {
        $text .= $r."<ul>".$r;
    } else {
        $text .= "</li>".$r;
    }
...
function GetActionsCats()
{
    ...
    for ($i = 1; $i <= $data["total"]; $i++): @mysql_data_seek(
      ...
      ($i - 1)
        ...
        $items[$idr]["pic"] = $ar["pic"];
        $items[$idr]["stat"] = $ar["stat"];
        $items[$idr]["link"] = "/".$link."/actions-r/".$ar["id"]."/"; endfor;
    $stotal = $data["total"] + 1;
    GetChild_(0);
    $text .= "<ul class='Catalog'>".$itext."</ul>";
...
function GetConsultsCats()
{
    ...
    $prlvl = 0;
    $data = DB(
      "SELECT ".$table.".*, COUNT(`".$table2."`.id) AS `cnt` FROM `".$table."` LEFT JOIN `".$table2."` ON `".$table2."`.`consultscats` LIKE CONCAT('%',',',".$table.".id,',','%') WHERE (".$table.".`stat`=1 && ".$table.".`type`=2) GROUP BY 1 ORDER BY ".$table.".`name`"
    );
    $items[0]["id"] = 0;
    $items[0]["pid"] = 0;
    for ($i = 1; $i <= $data["total"]; $i++): @mysql_data_seek(
      ...
      ($i - 1)
        ...
        $items[$idr]["pic"] = $ar["pic"];
        $items[$idr]["stat"] = $ar["stat"];
        $items[$idr]["link"] = "/".$link."/consult/".$ar["id"]."/"; endfor;
    $stotal = $data["total"] + 1;
    GetChild_C(0);
    $text .= "<ul class='ConsultsCatalog'>".$itext."</ul>";
...
{
    global $VARS, $GLOBAL, $ROOT, $RealPage, $dir, $Page, $node, $table, $table4, $C, $C10, $C15, $C25, $C5;
    if ($action["todata"]) {
        $d = ToRusData($action["todata"]);
    }
    $text = '<link media="all" href="/modules/standart/multiupload/client/uploader2.css" type="text/css" rel="stylesheet"><script type="text/javascript" src="/modules/standart/multiupload/client/uploader.js"></script>';
    ...
    if ($action['pic']) {
        $text .= '<span class="imgCon"><img src="/userfiles/picpreview/'.$action['pic'].'" class="img" /><img src="/template/standart/exit.png" class="remove" onclick="imgRemove($(this))" /><input type="hidden" name="pic" value="'.$action['pic'].'" /></span>';
    }
    $text .= '</div></td></tr>';
...
function addAction()
{
    ...
    if (isset($_SESSION['Data']["ActionButton"])) {
        ...
        if (trim($P['name']) == '') {
            ...
            $ar = explode(".", $P["todata"]);
            $sdata1 = mktime(0, 0, 0, $ar[1], $ar[0], $ar[2]);
            $pics = $pic = "";
            ...
            $q = "INSERT INTO `$table4` (`data`, `todata`, `pid`, `name`, `text`, `pic`, `pics`, `fas`, `rest`) VALUES ('".time(
              )."', '$sdata1', '".$dir[3]."', '".$name."', '".$text."', '$pic', '$pics', '".$fas."', '".$rest."')";
            DB($q);
            $last = DBL();
            DB(
              "UPDATE `$table4` SET `rate`='".$last."' WHERE  (id='".$last."')"
            );
        ...
        if (trim($P['name']) == '') {
            ...
            if ($pic != $P["pic"]) {
                if ($pic) {
                    foreach ($GLOBAL['AutoPicPaths'] as $path => $size) {
                        @unlink($ROOT."/userfiles/".$path."/".$pic);
                    }
                }
                $pic = $P["pic"];
                if ($pic) {
            ...
            if ($pics) {
                ...
                foreach ($pics as $key => $pici) {
                    if (!in_array($pici, $P["attachment"])) {
                        foreach ($GLOBAL['AutoPicPaths'] as $path => $size) {
                            @unlink($ROOT."/userfiles/".$path."/".$pici);
                        }
                        unset($pics[$key]);
                    }
                }
            ...
            if ($P["attachment"]) {
                foreach ($P["attachment"] as $pici) {
                    if (is_array($pics) && in_array($pici, $pics)) {
                        continue;
                    }
                    actionsResizePhoto($pici);
                    $pics[] = $pici;
...
function addQuestion()
{
    ...
    if (isset($_SESSION['Data']["QuestionButton"])) {
        ...
        if (trim($P['name']) == '' || trim(
            $P['text']
          ) == '' || (isset($_SESSION["CaptchaCode"]) && strtolower(
              $P['captcha']
            ) != strtolower($_SESSION["CaptchaCode"]))) {
            $msg = '<div class="ErrorDiv">Ошибка! Поля не заполнены или заполнены неверно</div>';
        } else {
            if (sizeof($matches[0]) < 3) {
                $msg = '<div class="ErrorDiv">Ошибка! Вопрос не имеет информативного содержания</div>';
            } else {
                ...
                if ($P["attachment"]) {
                    foreach ($P["attachment"] as $pici) {
                        ...
                        list($w, $h) = getimagesize(
                          $ROOT."/userfiles/temp/".$pici
                        );
                        list($sw, $sh) = explode(
                          "-",
                          $GLOBAL['AutoPicPaths']['picnews']
                        );
                        $k = min($w / $sw, $h / $sh);
                        $x = round(($w - $sw * $k) / 2);
                        $y = round(($h - $sh * $k) / 2);
                        crop(
                          $ROOT."/userfiles/temp/".$pici,
                          $ROOT."/userfiles/picnews/".$pici,
                          array($x, $y, round($sw * $k), round($sh * $k))
                        );
                        resize(
                          $ROOT."/userfiles/picnews/".$pici,
                          $ROOT."/userfiles/picnews/".$pici,
                        ...
                        rename(
                          $ROOT."/userfiles/temp/".$pici,
                          $ROOT."/userfiles/picoriginal/".$pici
                        );
                    }
                }
                $q = "INSERT INTO `$table5` (`data`, `name`, `text`, `rid`, `pics`) VALUES ('".time(
                  )."', '".$name."', '".$text."', '".(int)$dir['2']."', '".$pics."')";
                DB($q);
                $last = DBL();
                $msg = '<div class="SuccessDiv">Спасибо! Ваш вопрос отправлен</div>';

                $data = DB(
                  "SELECT _users.mail, ".$table.".id, ".$table.".name, ".$table2.".id AS cid, ".$table2.".name AS cname FROM ".$table." LEFT JOIN ".$table2." ON ".$table2.".consultscats LIKE '%,".$dir[2].",%' LEFT JOIN `_users` ON `_users`.id=".$table2.".uid WHERE (".$table.".`id`=".$dir[2].") GROUP BY 1"
                );
                if ($data["total"]) {
                    for ($i = 0; $i < $data["total"]; $i++) {
                        @mysql_data_seek($data["result"], $i);
...
function addAnswer($item)
{
    ...
    if (isset($_SESSION['Data']["AnswerButton"])) {
        $P = $_SESSION['Data'];
        if (trim($P['text']) == '' || !$P['cid']) {
            $msg = '<div class="ErrorDiv">Ошибка! Поля не заполнены или заполнены неверно</div>';
        } else {
            @require($ROOT."/modules/standart/ImageResizeCrop.php");
            if (!$P['name'] && $P['spec']) {
                $data = DB(
                  "SELECT `id`,`name`,`pic` FROM `".$table5."` WHERE (id=".$P['spec'].") LIMIT 1"
                );
                @mysql_data_seek($data["result"], 0);
                $ar = @mysql_fetch_array($data["result"]);
                $name = $ar['name'];
            ...
            $text = str_replace("'", "\'", $P['text']);
            $q = "INSERT INTO `$table5` (`data`, `name`, `text`, `pic`, `pid`, `cid`, `rid`, `pics`) VALUES ('".time(
              )."', '".$name."', '".$text."', '".$pic."', ".$item['id'].", ".$P['cid'].", ".$item['rid'].", '".$pics."')";
            DB($q);
            SD();
    ...
    if ($_SESSION['userid']) {
        $data = DB(
          "SELECT `id`,`name` FROM `".$table2."` WHERE (uid=".$_SESSION['userid']." && consultscats LIKE '%,".$item["rid"].",%')"
        );
        if ($data["total"]) {
            $comps = array();
            for ($i = 0; $i < $data["total"]; $i++) {
        ...
        $data2 = DB(
          "SELECT `id`,`name` FROM `".$table5."` WHERE (cid IN (".$compsStr.")) GROUP BY `name`"
        );
        if ($data2["total"]) {
            $specs = array();
            for ($i = 0; $i < $data2["total"]; $i++) {
    ...
    $text .= '<tr class="TRLine0"><td class="VarText" style="vertical-align:top; padding-top:10px;">Прикрепить изображения</td><td class="LongInput"><div class="uploaderCon"><div class="uploader" data-name="attachment[]" data-multiple="1"></div><div class="Info">Вы можете загрузить фотографию в формате jpg, gif и png</div></div><div class="uploaderFiles"></div></td></tr>';
    $text .= '<tr class="TRLine0"><td class="VarText">Компания</td><td class="LongInput"><select name="cid">'.GetSelected(
        $comps
      ).'</select></td></tr>';
    if ($specs) {
        $text .= '<tr class="TRLine0" style="display:table-row;"><td class="VarText" style="vertical-align:top; padding-top:10px;">Специалист</td><td class="LongInput"><select name="spec" onchange="ShowSets(this);">'.GetSelected(
            $specs,
            0
          ).'<option value="0">Добавить нового...</option></select>';
        $hide = ' ShowSets';
    }
...
function editAnswer($item)
{
    ...
    if (isset($_SESSION['Data']["AnswerButton"])) {
        ...
        if (trim($P['text']) == '' || !$P['cid']) {
            ...
            if ($P["attachment"]) {
                foreach ($P["attachment"] as $pici) {
                    $pics .= $pics ? "|".$pici : $pici;
                    if (is_array($pics_) && in_array($pici, $pics_)) {
                        continue;
                    }
                    list($w, $h) = getimagesize($ROOT."/userfiles/temp/".$pici);
                    list($sw, $sh) = explode(
    ...
    if ($_SESSION['userid']) {
        ...
        $data2 = DB(
          "SELECT `id`,`name` FROM `".$table5."` WHERE (cid IN (".$compsStr.")) GROUP BY `pic`"
        );
        if ($data2["total"]) {
            $specs = array();
            for ($i = 0; $i < $data2["total"]; $i++) {
...
function actionsResizePhoto($pic)
{
    ...
    $ext = str_replace(
      "jpeg",
      "jpg",
      strtolower(substr($pic, 1 + strrpos($pic, ".")))
    );

    foreach ($GLOBAL['AutoPicPaths'] as $path => $size) {
        if (!is_dir($ROOT."/userfiles/".$path)) {
            mkdir($ROOT."/userfiles/".$path, 0777);
        }
