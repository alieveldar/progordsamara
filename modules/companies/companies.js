...
function initMap(id, contacts) {
    ...
    for (i in gen_coords.longitudes) {
        longitude += parseFloat(gen_coords.longitudes[i]);
        latitude += parseFloat(gen_coords.latitudes[i]);
    }
...
function ItemDelete(id, tab, pid) {
    ActionAndUpdate(id, "DEL", tab, pid);
}

function ActionAndUpdate(id, act, tab, pid) {
    CloseBlank();
    JsHttpRequest.query('/modules/companies/qa-JSReq.php', {
        'id': id,
        'act': act,
        'tab': tab
        ...
        if (result) {
            if (act == "DEL") {
                $("#Act" + id + " a:last").remove();
                $("#Act" + id + " a:first").attr('href', $("#Act" + id + " a:first").attr('href').replace('edit/' + id, 'add/' + pid)).find('img').attr('src', $("#Act" + id + ' img').attr('src').replace('edit', 'add')).attr('title', 'Ответить');
            }
        }
    }, true);
}
