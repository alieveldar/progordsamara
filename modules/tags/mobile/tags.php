...
if ($start === 0) {
    ...
    $data = DB(
      "SELECT `name` FROM `".$table."` WHERE (`id`='".(int)$dir[1]."') LIMIT 1"
    );
    if ($data["total"] == 1) {
    ...
    for ($i = 0; $i < $data["total"]; $i++) {
        ...
        if ($ar["pic"] != "") {
            if (strpos($ar["pic"], "old") != 0) { /*Старый вид картинок*/
                ...
                $pic = "<img src='/userfiles/pictavto/".$ar["pic"]."' title='".$ar["name"]."' style='width:25%; float:left; margin-right:10px; ' />";
            }
        }
        if ($ar["uid"] != 0 && $ar["nick"] != "") {
        ...
        $text .= "<div class='NewsLentaList' style='overflow:hidden;'><a href='/".$ar["link"]."/view/".$ar["id"]."'>".$pic."</a><a href='/".$ar["link"]."/view/".$ar["id"]."'>".$ar["name"]."</a><br>
		".Replace_Data_Days($d[4]).",  Автор: ".$auth."</div>";
        $text .= $C25;
        //if($data["total"]>($i+1)){ if (($i+1)%$onblock==0) { $text.=$C25."<div class='banner2' style='margin-left:10px;' id='Banner-6-".(floor($i/$onblock)+1)."'></div>".$C; } else { }}
    }
