...
if ($HTTPREFERER == $_SERVER['SERVER_NAME']) {
    ...
    $q = DB(
      "SELECT `id` FROM `$table3` WHERE (($or) AND `pid`=$pid AND `link`='$link' AND `data`>".$GLOBAL["tonight"].")"
    );
    ...
    if ($q["total"] >= 1) {
        ...
        DB(
          "INSERT INTO `$table3` (`link`,`vid`,`uid`,`data`,`ip`,`pid`) VALUES ('$link', $vid, ".$uid.", '".$GLOBAL["now"]."', '".$GLOBAL["ip"]."', '$pid')"
        );

        $data = DB(
          "SELECT `".$table2."`.`id`, COUNT(`".$table3."`.`id`) as `cnt` FROM `".$table2."` LEFT JOIN `".$table3."` ON `".$table3."`.`vid`=`".$table2."`.`id`
