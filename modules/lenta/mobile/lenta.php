...
if ($start == "view") {
    ...
    $data = DB(
      "SELECT `comments`,`alttext`, `promo` FROM `".$table."` WHERE (`id`='".(int)$dir[2]."' ".$where.") LIMIT 1"
    );
    if ($data["total"] == 1) {
...
function GetLentaId()
{
    global $VARS, $GLOBAL, $dir, $RealHost, $Page, $node, $table, $table2, $table3, $table4, $table8, $table5, $table6, $table7, $link, $C, $C5, $C10, $C15, $C20, $ROOT;
    ...
    ### Основной запрос
    $data = DB(
      "SELECT `".$table."`.*, `".$dir[0]."_cats`.`name` as `ncat`, `_users`.`nick`, `_users`.`avatar`, `$table5`.`id` as `vvid` FROM `".$table."`
    ...
    ### Фотография
    if ($item["pic"] != "") {
        if (strpos($item["pic"], "old") != 0) { /*Старый*/
            $old = 1;
            $path = '/'.$item["pic"];
            $pic = "<div class='LentaPictureOld' title='".$cap."'><img src='".$item["pic"]."' title='".$cap."' /></div>";
        } else {
            /*Новый*/
            $old = 0;
            $path = '/userfiles/picitem/'.$item["pic"];
            $pic .= "<div class='LentaPicture-picitem' title='$cap'><img src='/userfiles/picitem/".$item["pic"]."' title='$cap' /><div class='LentaPicture-Cens'><h1>".$item["cens"]."</h1></div></div>";
        }
        if ($item["picauth"] != "") {
            $pic .= "<div class='PicAuth'>Автор фотографии: ".$item["picauth"]."</div>";
        }
    }
    ### Основной текст
    ...
    if ($lid != "") {
        if ($old == 1) {
            $lid = "<div class='ItemLidOld'>".$lid."</div>".$C10;
        } else {
            $lid = "<div class='ItemLid'>".$lid."</div>".$C10;
        }
    }
    ### Фото-отчет
    $p = DB(
    ...
    $report = $C."";
    if ($p["total"] > 0) {
        $report .= "<table class='ItemOrder'>";
        for ($i = 0; $i < $p["total"]; $i++): mysql_data_seek($p["result"], $i);
            $ar = @mysql_fetch_array($p["result"]);
            if ($ar["sets"] == 0) {
                $report .= "<tr><td width='30%' valign='top' class='type".$ar["sets"]."'><a href='/userfiles/picoriginal/".$ar["pic"]."' title='".$ar["name"]."' rel='prettyPhoto[gallery]'><img src='/userfiles/picpreview/".$ar["pic"]."' title='".$ar["name"]."' alt='".$ar["name"]."'></a></td><td width='70%' valign='top' class='type".$ar["sets"]."'><h4>".$ar["name"]."</h4>".$ar["text"]."</td></tr>";
            } else {
    ...
    if ($p["total"] > 0) {
        $album = "<h2>Фотоальбом:</h2><div class='ItemAlbum'>";
        for ($i = 0; $i < $p["total"]; $i++): mysql_data_seek($p["result"], $i);
            $ar = @mysql_fetch_array($p["result"]);
            $album .= "<a href='/userfiles/picoriginal/".$ar["pic"]."' title='".$ar["name"]."' rel='prettyPhoto[gallery]'><img src='/userfiles/picnews/".$ar["pic"]."' title='".$ar["name"]."' alt='".$ar["name"]."'></a>"; endfor;
        $album .= "</div>".$C;
    }
    ### Голосование
    if ((int)$item["vvid"] != 0) {
    ...
    if ($item["uid"] != 0 && $item["nick"] != "") {
        $auth = $avatar."Автор: <a href='http://".$VARS["mdomain"]."/users/view/".$item["uid"]."/'>".$item["nick"]."</a><br><b>".$d[1]."</b>";
    } else {
        $auth = "<img src='/userfiles/avatar/no_photo.jpg' />Автор: <a href='http://".$VARS["mdomain"]."/add/2/'>Народный корреспондент</a><br><b>".$d[1]."</b>";
    }
    ### Тэги и ошибки
    $t = trim($item["tags"], ",");
    $tags = "";
    ...
    $tags .= "<br><a href='http://".$VARS["mdomain"]."/newsv2/".$item["id"].".html'>Старый формат новостей</a>";
    ### Лого и контакты
    $cdata = DB(
    ...
    if ($cdata["total"]) {
        ...
        if ($con['name']) {
            ...
            $contacts .= "<h3>".$con["name"]."</h3><p class='contacts'><img src='/template/standart/address.png' style='vertical-align:middle;' />";
            if ($con["address"]) {
            ...
            $contacts .= "</p>".$con["anonce"].$C.'</div>';
        }
    }
    ### Лайки
    $likes = "<div class='Likes'>".Likes(
        Hsc($cap),
        "",
        "http://".$RealHost.$path,
        Hsc(strip_tags($lid))
    ...
    if ($item["pay"] != "") {
        $mixblock .= $C10."<div class='WhiteBlock PayBlock'>".$item["pay"]."</div>";
    }
    ### Платные ссылки
    if ($item["adv"] != "") {
        $mixblock .= $C20."<div class='CBG'></div>".$C5."<div class='AdvBlock'>".$item["adv"]."</div>".$C;
    }
    $mixblock .= '
<div id="smi_teaser_11730">
    <center>
        <a href="https://24smi.info/?utm_source=informer_11730">
...
<script type="text/JavaScript" encoding="utf8">
 ...
 sm.async = true;
 sm.src = "//jsn.24smi.net/7/d/11730.js";
 var s = document.getElementsByTagName("script")[0];
 s.parentNode.insertBefore(sm, s);})();
</script>';
    ### Новости по таким же тэгам
    $dtags .= "<h3>Будь в теме:</h3>";
    $dtags .= TheNewestInKazan(3, $item["id"]);
    ...
    */
    ...
    if ($item["endtext"] != "") {
        $endtext = $C5."<div class='WhiteBlock EndText'>".$item["endtext"]."</div>".$C;
    }
    ### Текст вывода

    if ($old == 1) {
        $text = "<h1>".$cap."</h1>"."<div class='WhiteBlock LentaItem'>".$lid.$pic.$ban1."<div class='ItemText'>".$maintext."</div>".$cards.$ban2.$report.$video.$album.$endtext.$voting.$event.$contacts.$C10.$likes.$mixblock.$ban3."</div>";
    } else {
        $text = "<h1>".$cap."</h1><div class='WhiteBlock LentaItem' style='padding:9px;'>".$pic.$C15.$lid.$ban1."<div class='ItemText'>".$maintext.$cards.$ban."</div>".$ban2.$report.$video.$album.$endtext.$voting.$event.$contacts.$C10.$likes.$mixblock.$dtags.$ban3."</div>";
    }
...
function GetLentaCat()
{
    ...
    for ($i = 0; $i < $data["total"]; $i++) {
        ...
        $pic = "";
        if ($ar["pic"] != "") {
            if (strpos($ar["pic"], "old") != 0) {
                $pic = "<a href='/".$dir[0]."/view/".$ar["id"]."'><img src='".$ar["pic"]."' title='".$ar["name"]."' /></a>";
            } else {
                /*Новый*/
        ...
        if ($UserSetsSite[3] == 1 && $ar["comments"] != 2) {
            $coms = "<a href='/".$dir[0]."/view/".$ar["id"]."#comments'>Комментарии</a>: <b>".$ar["comcount"]."</b>";
        } else {
            $coms = "";
        }
        ...
        if (($i + 1) % $onblock == 0) {
            $text .= "</div>".$C10."<div class='banner' id='Banner-29-".(floor(
                  $i / $onblock
                ) + 1)."'></div>".$C10."<div class='WhiteBlock'>";
        } else {
            $text .= $C25;
        }
    }
