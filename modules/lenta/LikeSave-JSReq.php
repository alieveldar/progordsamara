...
if ($HTTPREFERER == $_SERVER['SERVER_NAME']) {
    ...
    if ($R["link"] == $link && $pid != 0) {
        $data = DB(
          "SELECT `id` FROM `_likes` WHERE (`link`='".$link."' && `pid`='".$pid."' && `ip`='".$ip."' && `data`>'".(time(
            ) - 1 * 24 * 60 * 60)."')"
        );
        $user = (int)$data["total"];

        $result["log"] = "SELECT `id` FROM `_likes` WHERE (`link`='".$link."' && `pid`='".$pid."' && `ip`='".$ip."' && `data`>'".(time(
            ) - 1 * 24 * 60 * 60)."') [".$user."]";

        if ($user == 0) {
            # update likes and dislikes
            if ($ip == trim("94.180.249.218")) {
                $ip = "My.".rand(100000, 999999999);
            }
            DB(
              "INSERT INTO `_likes` (`data`,`ip`,`link`,`pid`,`like`) VALUES ('".time(
              )."','".$ip."','".$link."','".$pid."','".$qid."')"
            );
            if ($qid == 0) {
                DB(
                  "UPDATE `".$link."_lenta` set `dislikes`=`dislikes`+1 WHERE (`id`='".$pid."') LIMIT 1"
                );
            }
            if ($qid == 1) {
                DB(
                  "UPDATE `".$link."_lenta` set `likes`=`likes`+1 WHERE (`id`='".$pid."') LIMIT 1"
                );
            }
        }
        # get likes and dislikes
        $data = DB(
          "SELECT `likes`,`dislikes` FROM `".$link."_lenta` WHERE (`id`='".$pid."') LIMIT 1"
        );
