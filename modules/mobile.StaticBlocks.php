...
if ($SubDomain == 100 || $link == "index" || $link == "new-index") {
    ...
    if (is_file(
      "modules/page_mods/right_block/new-".$Domains[$SubDomain].".php"
    )) {
        ...
        $GLOBAL["log"] .= "<u>Подключение PHP</u>: правый блок не подключен<hr>";
    }
}
...
if ($SubDomain == 100 || $link == "index" || $link == "new-index") {
    ...
    if (is_file(
      "modules/page_mods/left_block/new-".$Domains[$SubDomain].".php"
    )) {
        ...
        $GLOBAL["log"] .= "<u>Подключение PHP</u>: левый блок не подключен<hr>";
    }
}
...
if ($SubDomain == 100 || $link == "index" || $link == "new-index") {
    ...
    if (is_file(
      "modules/page_mods/top_block/new-".$Domains[$SubDomain].".php"
    )) {
        ...
        $GLOBAL["log"] .= "<u>Подключение PHP</u>: верхний блок не подключен<hr>";
    }
}
...
if ($SubDomain == 100) {
    ...
    if (is_file(
      "modules/page_mods/bottom_block/new-".$Domains[$SubDomain].".php"
    )) {
        ...
        $GLOBAL["log"] .= "<u>Подключение PHP</u>: нижний блок не подключен<hr>";
    }
}
...
if (RetCache($file, 'cacheblock') == 'true') {
    list($Page["MainInformers"], $cap) = GetCache($file, 0);
} else {
    list($Page["MainInformers"], $cap) = GetMainInformers();
    SetCache($file, $Page["MainInformers"], '', 'cacheblock');
}

function GetMainInformers()
{
    global $C5;
//	$xml=simplexml_load_file("http://export.yandex.ru/bar/reginfo.xml?region=51"); $lvl=$xml->traffic->level; $icnt=$xml->traffic->icon; $txtt=$xml->traffic->hint; $temp=$xml->weather->day->day_part->temperature; $icnpa=$xml->weather->day->day_part->xpath('//image-v3'); $txtp=$xml->weather->day->day_part->weather_type; $icnp=$icnpa[0]; if ($lvl==1) { $balls=$lvl." балл"; } elseif ($lvl==2 || $lvl==3 || $lvl==4) { $balls=$lvl." балла"; } else { $balls=$lvl." баллов"; }
    ...
    $usdold = $xml->Central_Bank_RF->USD->Old->Exch_Rate;
    $usdch = round((float)$usd - (float)$usdold, 2);
    if ((float)$usdch < 0) {
        if (strlen($usdch) < 5) {
            $usdch = $usdch."0";
        }
        $usdch = "<span style='color:red !important;'>".$usdch."</span>";
    } else {
        if (strlen($usdch) < 4) {
            $usdch = $usdch."0";
        }
        $usdch = "<span style='color:green !important;'>+".$usdch."</span>";
    }
    ...
    $euroold = $xml->Central_Bank_RF->EUR->Old->Exch_Rate;
    $euroch = round((float)$euro - (float)$euroold, 2);
    if ((float)$euroch < 0) {
        if (strlen($euroch) < 5) {
            $euroch = $euroch."0";
        }
        $euroch = "<span style='color:red !important;'>".$euroch."</span>";
    } else {
        if (strlen($euroch) < 4) {
            $euroch = $euroch."0";
        }
        $euroch = "<span style='color:green !important;'>+".$euroch."</span>";
    }
    ...
    $text .= "<div class='Item'><a href='/advertise' target='_blank'><img src='/template/standart/icons/progorod.png' style='margin:0; width:170px; height:60px; border-radius:4px;' /></a></div>";

    return array("<div id='MainInformers'>".$text."</div>", "");
}
...
function getLentasOnModules()
{
    global $lentas;
    if (sizeof($lentas) == 0) {
        $modules = array("lenta", "concurs", "tatbrand");
        $q = "SELECT `link` FROM `_pages` WHERE (`module` IN ('".implode(
            "','",
            $modules
          )."')) LIMIT 50";
        $data = DB($q);
...
{
    return (str_replace(
      array(date("d.m.Y"), date("d.m.Y", time() - 60 * 60 * 24)),
      array("Сегодня", "Вчера"),
      $data
    ));
}
...
function getNewsFromLentas($q = '', $endq = '')
{
    ...
    foreach ($lentas as $l => $t) {
        $usedtext = "";
        if (sizeof($used[$l]) > 0) {
            $usedtext = " && `".$t."`.`id` NOT IN (0, ".implode(
                ",",
                $used[$l]
              ).")";
...
function DrawBubrItem($ar)
{
    if ($ar["name"] != "") {
        $pic = "";
        $text = "";
        $pic = "<img src='".$ar["pic"]."' title='".$ar["name"]."' alt='".$ar["name"]."' />";
        $text .= "<div class='itemlist ".$class."'><noindex><a href='".$ar["link"]."' rel='nofollow'>";
        if ($ar["pic"] != "") {
            $text .= $pic;
        }
        $text .= $ar["name"]."</a>";
...
function DrawNewsItem($ar, $datas = '', $class = '')
{
    ...
    $text .= "<div class='itemlist ".$class."'><a href='/".$ar["link"]."/view/".$ar["id"]."'>";
    if ($ar["pic"] != "") {
        $text .= $pic;
    }
    $text .= $ar["name"]."</a>";
...
{
    $lentas = getLentasOnModules();
    $q = "SELECT `[table]`.`id`, `[table]`.`name`, `[table]`.`data`, `[table]`.`comcount`, `[table]`.`pic`, '[link]' as `link` FROM `[table]`
	WHERE (`[table]`.`stat`='1' && `[table]`.`onind`=1 && `[table]`.`id`!=".$not." && `[table]`.`data`>'".(time(
        ) - 24 * 60 * 60)."')";
    $endq = "ORDER BY RAND() LIMIT ".$limit;
    $tv = getNewsFromLentas($q, $endq);
    $text .= "<newsblock>";
    $onblock = 4;
    for ($i = 0; $i < $tv["total"]; $i++): @mysql_data_seek($tv["result"], $i);
        $ar = @mysql_fetch_array($tv["result"]);
        $text .= DrawNewsItem($ar, 0); endfor;
    $text .= "</newsblock>";
...
{
    $lentas = getLentasOnModules();
    $q = "SELECT `[table]`.`id`, `[table]`.`name`, `[table]`.`data`, `[table]`.`comcount`, `[table]`.`pic`, '[link]' as `link` FROM `[table]` WHERE (`[table]`.`stat`='1' && `[table]`.`promo`=1)";
...
function ShowProAutoBlock($limit = 3, $from = 0)
{
    global $C5;
    $tv = DB(
      "SELECT `id`,`name`,`pic`,`comcount`,`data`, 'auto' as `link` FROM `auto_lenta` WHERE (`cat`=2 && `stat`=1) ORDER BY `data` DESC LIMIT ".$from.",".$limit
    );
    $text .= "<newsblock>";
    for ($i = 0; $i < $tv["total"]; $i++): @mysql_data_seek($tv["result"], $i);
        $ar = @mysql_fetch_array($tv["result"]);
        if ($i == 0) {
            $text .= "<div class='itemlist'><a href='/$ar[link]/view/$ar[id]' style='font-size:13px; font-weight:bold;'><img src='/userfiles/pictavto/$ar[pic]' style='border:none; width:200px; height:120px;'>".$C5."$ar[name]</a></div>";
        } else {
            $text .= DrawNewsItem($ar);
        } endfor;
    $text .= "</newsblock>";
...
function ShowStreetFashioBlock($limit = 3, $from = 0)
{
    global $C5;
    $tv = DB(
      "SELECT `id`,`name`,`pic`,`comcount`,`data`, 'news' as `link` FROM `news_lenta` WHERE (`cat`=5 && `stat`=1) ORDER BY `data` DESC LIMIT ".$from.",".$limit
    );
    $text .= "<newsblock>";
    for ($i = 0; $i < $tv["total"]; $i++): @mysql_data_seek($tv["result"], $i);
        $ar = @mysql_fetch_array($tv["result"]);
...
function ShowTopOnForumBlock($limit = 3, $from = 0)
{
    global $C5;
    $tv = DB(
      "SELECT `id`,`name`,`comcount`,`data`, 'live' as `link` FROM `live_lenta` WHERE (`stat`=1) ORDER BY `data` DESC LIMIT ".$from.",".$limit
    );
    $text .= "<newsblock>";
    for ($i = 0; $i < $tv["total"]; $i++): @mysql_data_seek($tv["result"], $i);
        $ar = @mysql_fetch_array($tv["result"]);
        $text .= DrawNewsItem($ar); endfor;
    $text .= "</newsblock>";
...
function razdelOldSamara()
{
    ...
    $table = array(); #tag 54
    $lentas = getLentasOnModules();
    $q = "SELECT `[table]`.`id`, `[table]`.`name`, `[table]`.`data`, `[table]`.`comcount`, `[table]`.`pic`, '[link]' as `link` FROM `[table]` WHERE (`[table]`.`stat`='1' && `redak`=1 && `[table]`.`tags` LIKE '%,54,%')";
    $endq = "ORDER BY `data` DESC LIMIT 4";
    $tv = getNewsFromLentas($q, $endq);
    for ($i = 0; $i < $tv["total"]; $i++): @mysql_data_seek($tv["result"], $i);
        $ar = @mysql_fetch_array($tv["result"]);
        $table[] = $ar; endfor;
    $text .= "<h2><a href='/tags/54/'>Старая Самара</a></h2><div class='RedBlock'>";
    $incount = 1;
    foreach ($table as $ar): $d = ToRusData($ar["data"]);
        if ($incount == 1) {
            $text .= "<div class='FirstNews'><a href='/$ar[link]/view/$ar[id]'><img src='/userfiles/picnews/$ar[pic]' /><div class='Cap'>$ar[name]</h3></div></a><div class='Lid'>".CutText(
                $ar["lid"],
                100
              )."</div><div class='Data'>".ToLocalDay($d[4])."</div></div>";
        } else {
            $text .= "<div class='OtherNewsItem'><a href='/$ar[link]/view/$ar[id]'><img src='/userfiles/pictavto/$ar[pic]' /><div class='Cap'>$ar[name]</h3></div></a><div class='Data'>".ToLocalDay(
                $d[4]
              )."</div></div>";
        }
        $incount++; endforeach;
    $text .= "<a href='/tags/54' class='ReadMoreLink'>Читать больше »</a>";
    $text .= "</div>";
...
function razdelAfisha()
{
    ...
    $table = array(); #tag 89
    $lentas = getLentasOnModules();
    $q = "SELECT `[table]`.`id`, `[table]`.`name`, `[table]`.`data`, `[table]`.`comcount`, `[table]`.`pic`, '[link]' as `link` FROM `[table]` WHERE (`[table]`.`stat`='1' && `[table]`.`tags` LIKE '%,89,%')";
    $endq = "ORDER BY `data` DESC LIMIT 4";
    $tv = getNewsFromLentas($q, $endq);
    for ($i = 0; $i < $tv["total"]; $i++): @mysql_data_seek($tv["result"], $i);
        $ar = @mysql_fetch_array($tv["result"]);
        $table[] = $ar; endfor;
    $text .= "<h2><a href='/tags/89/'>Афиша</a></h2><div class='RedBlock'>";
    $incount = 1;
    foreach ($table as $ar): $d = ToRusData($ar["data"]);
        if ($incount == 1) {
            $text .= "<div class='FirstNews'><a href='/$ar[link]/view/$ar[id]'><img src='/userfiles/picnews/$ar[pic]' /><div class='Cap'>$ar[name]</h3></div></a><div class='Lid'>".CutText(
        ...
        $incount++; endforeach;
    $text .= "<a href='/tags/89' class='ReadMoreLink'>Читать больше »</a>";
    $text .= "</div>";
...
function razdelUspeh()
{
    ...
    $table = array();
    $tv = DB(
      "SELECT `id`,`name`,`pic`,`comcount`,`data`, 'business' as `link` FROM `business_lenta` WHERE (`cat`=5 && `stat`=1 && `redak`=1) ORDER BY `data` DESC LIMIT 4"
    );
    for ($i = 0; $i < $tv["total"]; $i++): @mysql_data_seek($tv["result"], $i);
        $ar = @mysql_fetch_array($tv["result"]);
        $table[] = $ar; endfor;
    $text .= "<h2><a href='/business/cat/5'>Школа успеха</a></h2><div class='RedBlock'>";
    $incount = 1;
    foreach ($table as $ar): $d = ToRusData($ar["data"]);
        if ($incount == 1) {
            $text .= "<div class='FirstNews'><a href='/$ar[link]/view/$ar[id]'><img src='/userfiles/picnews/$ar[pic]' /><div class='Cap'>$ar[name]</h3></div></a><div class='Lid'>".CutText(
        ...
        $incount++; endforeach;
    $text .= "<a href='/".$ar["link"]."/cat/5' class='ReadMoreLink'>Читать больше »</a>";
    $text .= "</div>";
...
function razdelWorldNewsIt()
{
    ...
    $table = array();
    $tv = DB(
      "SELECT `id`,`name`,`pic`,`comcount`,`data`, 'news' as `link` FROM `news_lenta` WHERE (`cat`=3 && `stat`=1 && `redak`=1) ORDER BY `data` DESC LIMIT 4"
    );
    for ($i = 0; $i < $tv["total"]; $i++): @mysql_data_seek($tv["result"], $i);
        $ar = @mysql_fetch_array($tv["result"]);
        $table[] = $ar; endfor;
    $text .= "<h2><a href='/news/cat/3'>Новости мира</a></h2><div class='RedBlock'>";
    $incount = 1;
    foreach ($table as $ar): $d = ToRusData($ar["data"]);
        if ($incount == 1) {
            $text .= "<div class='FirstNews'><a href='/$ar[link]/view/$ar[id]'><img src='/userfiles/picnews/$ar[pic]' /><div class='Cap'>$ar[name]</h3></div></a><div class='Lid'>".CutText(
        ...
        $incount++; endforeach;
    $text .= "<a href='/".$ar["link"]."/cat/3' class='ReadMoreLink'>Читать больше »</a>";
    $text .= "</div>";
...
{
    global $C, $C20, $C10, $C25, $VARS;
    $link = "brandsbattle";
    $table = $link."_lenta";
    $table2 = "_widget_pics";
    $table3 = "_widget_votes";
    $data = DB(
      "SELECT * FROM `$table` WHERE (`stat`=1) ORDER BY `data` DESC LIMIT 1"
    );
    @mysql_data_seek($data["result"], 0);
    $item = @mysql_fetch_array($data["result"]);
    $text .= "<h3>".$item["name"]."</h3>";
    $data = DB(
      "SELECT `".$table2."`.*, COUNT(`".$table3."`.`id`) as `cnt` FROM `".$table2."` LEFT JOIN `".$table3."` ON `".$table3."`.`vid`=`".$table2."`.`id` WHERE (`".$table2."`.`link`='".$link."' AND `".$table2."`.`pid`=".$item["id"]." AND `".$table2."`.`stat`=1) GROUP BY 1 ORDER BY `".$table2."`.`rate` ASC"
    );
    $items = array();
    $horblock = array();
    $total = $data["total"];
    ...
    $path = 'http://'.trim(
        $Domains[$item["domain"]].'.'.$VARS["mdomain"],
        '.'
      ).'/'.$link.'/view/'.$item['id'];
    $block = 0;
    $i = 1;
    foreach ($items as $ar) {
        ...
        $horblock[$block]["pics"] .= '<a href="javascript:void(0);" onclick=\'voteForm('.$ar["id"].', '.$item["id"].', "'.$link.'", "'.Hsc(
            $item["name"]
          ).'", "Я голосую за: '.Hsc(
            $ar["name"]
          ).'", "'.$path.'", "http://'.$VARS["mdomain"].'/userfiles/picpreview/'.$ar['pic'].'")\'>Голосовать</a>';
        $horblock[$block]["pics"] .= '</div></td>';
        if ($i % 3 == 0) {
            $block++;
        }
        $i++;
    }
    foreach ($horblock as $block) {
        $content .= "<tr>".$block["names"]."</tr><tr>".$block["pics"]."</tr><tr><td colspan='3'>".$C20.$C10."</td></tr>";
    }
...
{
    global $tab, $bannersq, $C, $C5, $C10, $C15, $C20, $C25;
    $text = '';
    $cats = implode(',', $array);
    $image_folder = "picintv";
    // ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ----
    $data = DB(
      "SELECT `".$tab."_lenta`.`id`,`".$tab."_lenta`.`gis`, `".$tab."_lenta`.`name`, `".$tab."_lenta`.`pic`, `".$tab."_lenta`.`cat`, `".$tab."_lenta`.`data`, `".$tab."_cats`.`name` as `ncat` FROM `".$tab."_lenta`
    ...
    for ($i = 0; $i < $data["total"]; $i++):
        ...
        $ar = @mysql_fetch_array($data["result"]);
        $d = ToRusData($ar["data"]);
        if ($column == 2 && ($i + 1 - ($column - 1) * $limit) == 2) {
            if ($bannersq == 0) {
                $text .= "<div id='Banner-20-1' class='Auto-Sq-Banner'></div>";
            } else {
                $text .= "<div id='Banner-21-".$bannersq."' class='Auto-Sq-Banner'></div>";
            }
            $bannersq++;
        }
        if ((int)$ar["gis"] == 1) {
            $glavnayapic = "<div class='GlavnayaTema'></div>";
            $name = "<a href='/".$tab."/view/".$ar["id"]."' title='".$ar["name"]."'><b>".$ar["name"]."</b></a>";
        } else {
            $glavnayapic = "";
            $name = "<a href='/".$tab."/view/".$ar["id"]."' title='".$ar["name"]."'>".$ar["name"]."</a>";
        }

        $text .= "<div class='ProAuto-Lenta'>";
        $pic = "<div class='NewsProBigPic'><a href='/".$tab."/view/".$ar["id"]."' title='".$ar["name"]."'><img src='/userfiles/".$image_folder."/".$ar["pic"]."' title='".$ar["name"]."' alt='".$ar["name"]."' />".$glavnayapic."</a></div>".$C5;
        if (($i % $limit) == 0) {
            $text .= "<div class='CategoryBig'><a href='/".$tab."/cat/".$ar["cat"]."'>".$ar["ncat"]."</a> ".$d[4]."</div>".$C5.$pic.$name;
        } else {
            $text .= "<div class='CategorySmall'><a href='/".$tab."/cat/".$ar["cat"]."'>".$ar["ncat"]."</a> ".$d[4]."</div>".$C5;
            if ($column == 1 && ($i + 1 - ($column - 1) * $limit) == 4) {
                $text .= $pic.$C;
            }
            if ($column == 3 && ($i + 1 - ($column - 1) * $limit) == 3) {
                $text .= $pic.$C;
            }
            if ($column == 4 && ($i + 1 - ($column - 1) * $limit) == 4) {
                $text .= $pic.$C;
            }
            $text .= $name;
        }
        $text .= "</div>"."<div class='Block-Line'></div>";
        if (($i + 1) % $limit == 0 && $column != 4) {
            if ($column != 3) {
                $text .= "</div><div class='MainColumn'>";
            } else {
                $text .= "</div><div class='MainColumn OnlyBig'>";
            }
            $column++;
        }
    endfor;
...
function Get_Pro_Test_Drive($array, $limit = 3)
{
    global $tab, $bannersq, $C, $C5, $C10, $C15, $C20, $C25;
    $text = '';
    $cats = implode(',', $array);
    $image_folder = "picintv";
    $image_folder2 = "picsquare";
    // ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ----
    $data = DB(
      "SELECT `".$tab."_lenta`.`id`, `".$tab."_lenta`.`name`, `".$tab."_lenta`.`lid`, `".$tab."_lenta`.`pic`, `".$tab."_lenta`.`cat`, `".$tab."_lenta`.`data`, `".$tab."_cats`.`name` as `ncat` FROM `".$tab."_lenta`
    ...
    for ($i = 0; $i < $data["total"]; $i++): @mysql_data_seek(
      ...
      $i
        ...
        if ($i == 0) {
            $data2 = DB(
              "SELECT `pic` FROM `_widget_pics` WHERE (`pid`='".$ar["id"]."' && `link`='".$tab."' && `stat`=1) LIMIT 5"
            );
            for ($j = 0; $j < $data2["total"]; $j++): @mysql_data_seek(
              $data2["result"],
              $j
            );
                $ap = @mysql_fetch_array($data2["result"]);
                $pics .= "<a href='/".$tab."/view/".$ar["id"]."' title='".$ar["name"]."'><img src='/userfiles/".$image_folder2."/".$ap["pic"]."' class='smallpic' /></a>"; endfor;
            $pic = "<a href='/".$tab."/view/".$ar["id"]."' title='".$ar["name"]."'><img src='/userfiles/".$image_folder."/".$ar["pic"]."' title='".$ar["name"]."' alt='".$ar["name"]."' class='bigpic' /></a>";
            $cat = "<div class='name'><a href='/".$tab."/cat/".$ar["cat"]."'>".$ar["ncat"]."</a></div>";
            $text .= "<div class='TestDrive'><div class='images'>".$cat.$pic.$C10.$pics."</div>";
            $text .= "<div class='text'><h2><a href='/".$tab."/view/".$ar["id"]."' title='".$ar["name"]."'>".$ar["name"]."</a></h2>".$C5.$ar["lid"].$C10."<div class='CategorySmall'>".$d[4]."</div></div>";
            $text .= "</div>";
        } else {
            if ($i == 1) {
                $text .= "<div class='TestDriveAnons'>";
            }
            $pic = "<a href='/".$tab."/view/".$ar["id"]."' title='".$ar["name"]."'><img src='/userfiles/".$image_folder."/".$ar["pic"]."' title='".$ar["name"]."' alt='".$ar["name"]."' /></a>";
            $text .= "<div class='item'>".$pic."<a href='/".$tab."/view/".$ar["id"]."' title='".$ar["name"]."'>".$ar["name"]."</a><div class='CategorySmall'>".$d[4]."</div>".$C."</div>";
            if ($i % 2 == 0 && $i != ($data["total"] - 1) && $i != 0) {
                $text .= $C."<div class='Block-Line'></div>";
            }
            if ($i == ($data["total"] - 1)) {
                $text .= $C."</div>";
            }
...
function Get_Pro_Blog_Block($users, $cat = 7)
{
    global $tab, $bannersq, $C, $C5, $C10, $C15, $C20, $C25;
    $text = '';
    $users = implode(',', $users);
    ...
    // ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ----
    $data = DB(
      "SELECT `".$tab."_lenta`.`id`, `".$tab."_lenta`.`uid` as `uid`, `".$tab."_lenta`.`name`, `".$tab."_lenta`.`cat`, `".$tab."_lenta`.`data`, `_users`.`nick`, `_users`.`avatar` FROM `".$tab."_lenta`
    ...
    if ($data["total"] > 0) {
        $text .= "<div><div class='MainColumn'><div class='BlogItem'><img src='/userfiles/avatar/img-20130320100404-521.jpg' />";
        $ar["name"] = "Отвечает Михаил Савин, заместитель командира полка ДПС ГИБДД";
    ...
    for ($i = 0; $i < $data["total"]; $i++): @mysql_data_seek(
      ...
      $i
        ...
        $d = ToRusData($ar["data"]);
        $pic = '';
        $pic = "<a href='/users/blogger/".$ar["uid"]."' title='".$ar["name"]."'><img src='/".$ar["avatar"]."' /></a>";
        $text .= "<div class='BlogItem'>".$pic;
        $text .= "<div class='name'><a href='/users/blogger/".$ar["uid"]."' title='".$ar["name"]."'>".$ar["nick"]."</a></div>";
        $text .= "<div class='text'><a href='/".$tab."/view/".$ar["id"]."' title='".$ar["name"]."'>".$ar["name"]."</a></div>";
        $text .= $C."</div>";
        if (($i + 1) % $limit == 0 && $column != 4) {
            if ($column != 3) {
                $text .= "</div><div class='MainColumn'>";
...
function MostProDiscussed($limit = 30)
{
    global $JSmodules, $CSSmodules, $SubDomain, $GLOBAL;
    $file = "agregator-".$SubDomain."_promostdiscussed";
    if (RetCache($file) == "true") {
        list($text, $cap) = GetCache($file, 0);
    } else {
        list($text, $cap) = CreateMostProDiscussed($limit);
        SetCache($file, $text, "");
    }
...
function CreateMostProDiscussed($limit)
{
    global $VARS, $GLOBAL, $Domains, $C, $C15, $C30;
    $query = '';
    $table = "auto";
    $text = '';
    $twoWeeksAgo = strtotime(date('Y-m-d H:i')) - (60 * 60 * 24 * 21);
    $data = DB(
      "SELECT `".$table."_lenta`.`id`,`".$table."_lenta`.`name`, `".$table."_lenta`.`cat`, `".$table."_lenta`.`comcount`, `".$table."_cats`.`name` as `catname`
    ...
    if ($data["total"] > 0) {
        $text .= '<div class="ProNewsBlock"><div class="scrollbarY"><div class="scrollbar"><div class="track"><div class="thumb"></div></div></div><div class="NewsBlockItems viewport"><div class="overview">';
        for ($i = 0; $i < $data["total"]; $i++) {
            @mysql_data_seek($data["result"], $i);
            $ar = @mysql_fetch_array($data["result"]);
            $path = '/'.$table.'/view/'.$ar["id"];
            $text .= '<a href="'.$path.'">'.$ar["name"].'</a>
	<div class="CategorySmall"><a href="/'.$table.'/cat/'.$ar["cat"].'">'.$ar["catname"].'</a> Комментарии: '.$ar["comcount"].'</div><div class="Block-Line"></div>';
        }
        $text .= '<div class="C25"></div></div></div></div></div>';
    }
...
function MostProNewest($limit = 5)
{
    global $JSmodules, $CSSmodules, $SubDomain, $GLOBAL;
    $file = "agregator-".$SubDomain."_promostNewest";
    if (RetCache($file) == "true") {
        list($text, $cap) = GetCache($file, 0);
    } else {
        list($text, $cap) = CreateMostProNewest($limit);
        SetCache($file, $text, "");
    }
...
function CreateMostProNewest($limit)
{
    global $VARS, $GLOBAL, $Domains, $C, $C15, $C30;
    $query = '';
    $table = "auto";
    $text = '';
    $data = DB(
      "SELECT `".$table."_lenta`.`id`,`".$table."_lenta`.`pic`,`".$table."_lenta`.`name`,`".$table."_lenta`.`data`, `".$table."_lenta`.`cat`, `".$table."_lenta`.`comcount`, `".$table."_cats`.`name` as `catname`
    ...
    if ($data["total"] > 0) {
        ...
        for ($i = 0; $i < $data["total"]; $i++) {
            ...
            $cap = $ar["catname"];
            if ($i == 0) {
                $pic = "<a href='".$path."'><img src='/userfiles/picintv/".$ar["pic"]."' title='".$ar["name"]."' alt='".$ar["name"]."' /></a>";
                $text .= '<item class="KazanFirst">'.$pic.'<a href="'.$path.'" class="FirstLink">'.$ar["name"].'</a><div class="CategorySmall FirstData">'.$ar["catname"].' : '.$d[4].'</div></item><div class="Block-Line"></div>';
            } else {
                $text .= '<item><a href="'.$path.'">'.$ar["name"].'</a><div class="CategorySmall"><a href="/'.$table.'/cat/'.$ar["cat"].'">'.$ar["catname"].'</a> '.$d[4].'</div></item><div class="Block-Line"></div>';
            }
        }
        $text .= '</div>';
    }

    return (array(Replace_Data_Days($text), $cap));
}

// ----------- ----------- ----------- ----------- ----------- ----------- ----------- ----------- ----------- ----------- ----------- ----------- ----------- ----------- ----------- ----------- ----------- -----------

function ProNewestCat($CAT = 0, $limit = 5)
{
    global $JSmodules, $CSSmodules, $SubDomain, $GLOBAL;
    $file = "agregator-".$SubDomain."_promostNewest".$CAT;
    if (RetCache($file) == "true") {
        list($text, $cap) = GetCache($file, 0);
    } else {
        list($text, $cap) = CreateProNewestCat($limit);
        SetCache($file, $text, "");
    }
...
function CreateProNewestCat($CAT = "0", $limit = 5)
{
    global $VARS, $GLOBAL, $Domains, $C, $C15, $C30;
    $query = '';
    $table = "auto";
    ...
    $catin = "";
    if ($CAT != "" && $CAT != 0) {
        $catin = " && `".$table."_lenta`.`cat` IN (0,".$CAT.")";
    }
    $data = DB(
      "SELECT `".$table."_lenta`.`id`,`".$table."_lenta`.`pic`,`".$table."_lenta`.`name`,`".$table."_lenta`.`data`, `".$table."_lenta`.`cat`, `".$table."_lenta`.`comcount`, `".$table."_cats`.`name` as `catname`
...
function ForumProDiscussed($limit = 30)
{
    global $JSmodules, $CSSmodules, $SubDomain, $GLOBAL;
    $file = "agregator-".$SubDomain."_proforumdiscussed";
    if (RetCache($file) == "true") {
        list($text, $cap) = GetCache($file, 0);
    } else {
        list($text, $cap) = CreateForumProDiscussed($limit);
        SetCache($file, $text, "");
    }
...
function CreateForumProDiscussed($limit)
{
    global $VARS, $GLOBAL, $Domains, $C, $C15, $C30;
    $query = '';
    $table = "live";
    $text = '';
    $twoWeeksAgo = strtotime(date('Y-m-d H:i')) - (60 * 60 * 24 * 60);
	...
	WHERE (`".$table."_lenta`.`stat`='1' && `".$table."_lenta`.`fid`=7 && `".$table."_lenta`.`data`>$twoWeeksAgo) GROUP BY 1 ORDER BY `".$table."_lenta`.`update` DESC LIMIT $limit";
    $data = DB($q);
    if ($data["total"] > 0) {
        $text .= '<div class="ProNewsBlock"><div class="scrollbarY"><div class="scrollbar"><div class="track"><div class="thumb"></div></div></div><div class="NewsBlockItems viewport"><div class="overview">';
        for ($i = 0; $i < $data["total"]; $i++) {
...
function getMaxComments($limit = 7)
{
    global $C, $C20;
    $lentas = getLentasOnModules();
    $dataold = time() - 7 * 24 * 60 * 60;
    $q = "SELECT `[table]`.`id`, `[table]`.`name`, `[table]`.`data`, `[table]`.`comcount`, '[link]' as `link` FROM `[table]` WHERE (`[table]`.`stat`='1' && `[table]`.`data`>'".$dataold."')";
    $endq = "ORDER BY `comcount` DESC LIMIT ".$limit;
    $tv = getNewsFromLentas($q, $endq);
    if ((int)$tv["total"] > 0) {
        for ($i = 0; $i < $tv["total"]; $i++): @mysql_data_seek(
          $tv["result"],
          $i
        );
            $ar = @mysql_fetch_array($tv["result"]);
            $text .= "<div class='ONew'><a href='/".$ar["link"]."/view/".$ar["id"]."'>".$ar["name"]."</a>".$C.Dater(
                $ar
              )."</div>".$C20; endfor;
    }
...
function getMaxSeens($limit = 7)
{
    ...
    $lentas = getLentasOnModules();
    $dataold = time() - 7 * 24 * 60 * 60;
    $q = "SELECT `[table]`.`id`,`[table]`.`seens`, `[table]`.`name`, `[table]`.`data`, `[table]`.`comcount`, '[link]' as `link` FROM `[table]` WHERE (`[table]`.`stat`='1' && `[table]`.`data`>'".$dataold."')";
    $endq = "ORDER BY `seens` DESC LIMIT ".$limit;
    $tv = getNewsFromLentas($q, $endq);
    if ((int)$tv["total"] > 0) {
        for ($i = 0; $i < $tv["total"]; $i++): @mysql_data_seek(
          $tv["result"],
...
function getMaxLikes($limit = 7)
{
    ...
    $lentas = getLentasOnModules();
    $dataold = time() - 7 * 24 * 60 * 60;
    $q = "SELECT `[table]`.`id`, `[table]`.`likes`, `[table]`.`name`, `[table]`.`data`, `[table]`.`comcount`, '[link]' as `link` FROM `[table]` WHERE (`[table]`.`stat`='1' && `[table]`.`data`>'".$dataold."')";
    $endq = "ORDER BY `likes` DESC LIMIT ".$limit;
    $tv = getNewsFromLentas($q, $endq);
    if ((int)$tv["total"] > 0) {
        for ($i = 0; $i < $tv["total"]; $i++): @mysql_data_seek(
          $tv["result"],
