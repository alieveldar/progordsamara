...
if ($start == "view") {
    ...
    $data = DB(
      "SELECT $table.`name`, $table.`text`, $table.`pic`, $table2.* FROM `$table` LEFT JOIN $table2 ON $table2.`pid`=".(int)$dir[2]." WHERE (`id`=".(int)$dir[2]." $where) LIMIT 1"
    );
    ...
    if ($data["total"]) {
        ...
        if (RetCache($file) == "true") {
            ...
            list($text, $cap) = GetHoroscopeId($item);
