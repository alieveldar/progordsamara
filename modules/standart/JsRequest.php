...
class JsHttpRequest
{
    ...
    var $_contentTypes = array(
      ...
      "xml" => "text/plain",
        // In XMLHttpRequest mode we must return text/plain - stupid Opera 8.0. :(
      "form" => "text/html",
      "" => "text/plain",
        // for unknown loader
    );
    ...
    function JsHttpRequest($enc)
    {
        ...
        if (preg_match(
          ...
          $m
            ...
            $this->_unicodeConvMethod = function_exists(
              'mb_convert_encoding'
            ) ? 'mb' : (function_exists('iconv') ? 'iconv' : null);
            ...
            ini_set(
              ...
              $this->_uniqHash.ini_get('error_prepend_string')
            );
            ini_set(
              'error_append_string',
              ini_get('error_append_string').$this->_uniqHash
            );
            ...
            $file = $line = null;
            $headersSent = version_compare(
              PHP_VERSION,
              "4.3.0"
            ) < 0 ? headers_sent() : headers_sent($file, $line);
            if ($headersSent) {
                trigger_error(
    ...
    function php2js($a = false)
    {
        ...
        if ($isList) {
            ...
            foreach ($a as $k => $v) {
                $result[] = JsHttpRequest::php2js(
                    $k
                  ).': '.JsHttpRequest::php2js($v);
            }
    ...
    function _correctSuperglobals()
    {
        ...
        $rawPost = strcasecmp(
          ...
          'POST'
        ) == 0 ? (isset($GLOBALS['HTTP_RAW_POST_DATA']) ? $GLOBALS['HTTP_RAW_POST_DATA'] : @file_get_contents(
          "php://input"
        )) : null;
        $source = array(
    ...
    function _obHandler($text)
    {
        ...
        $wasFatalError = false;
        if (preg_match_all(
          "/{$this->_uniqHash}(.*?){$this->_uniqHash}/sx",
          $text,
          $m
        )) {
            // Display_errors:
            ...
            $needRemoveErrorMessages = !ini_get(
                'display_errors'
              ) || (!$this->_prevDisplayErrors && ini_get(
                  'display_errors'
                ) == $this->_magic);
        ...
        if (function_exists('array_walk_recursive') && function_exists(
            'json_encode'
    ...
    function _toUtf8_callback(&$v, $k, $fromEnc)
    {
        ...
        if ($this->_toUtfFailed || !is_scalar($v) || strpbrk(
            $k,
            $this->_nonAsciiChars
          ) !== false) {
    ...
    function _ucs2EntitiesDecode($data)
    {
        if (is_array($data)) {
            $d = array();
            foreach ($data as $k => $v) {
                $d[$this->_ucs2EntitiesDecode($k)] = $this->_ucs2EntitiesDecode(
                  $v
                );
            }
            ...
            if (strpos($data, '%u') !== false) { // improve speed
                $data = preg_replace_callback(
                  '/%u([0-9A-F]{1,4})/si',
                  array(&$this, '_ucs2EntitiesDecodeCallback'),
                  $data
                );
            }
