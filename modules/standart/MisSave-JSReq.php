...
if ($HTTPREFERER == $_SERVER['HTTP_HOST']) {
    ...
    $result = array();
    DB(
      "INSERT INTO `$table` (`link`,`text`,`comment`,`data`) VALUES ('$link', '$text', '$comment', '".time(
      )."')"
    );
    $last = DBL();
    $result["Text"] = 'Спасибо! Ошибка будет исправлена!';
    $result["Code"] = 1;
    ...
    if ($ar["sets"] != "") {
        MailSend($ar["sets"], $subject, $body, $VARS["sitemail"]);
    }
