...
if (!isset($_SESSION['userreferer'])) {
    $_SESSION['userreferer'] = $_SERVER['HTTP_REFERER'];
}
...
if ($UserSetsSite[0] == 1) {
    ...
    if ((int)$_SESSION['userid'] == 0 && !preg_match(
        '/^m\./',
        $_SERVER['HTTP_HOST']
      )) {
        ### нет авторизации
        $GLOBAL["log"] .= "<b>нет авторизации</b><hr>";
        $UserAuthLine .= "<div id='UserAuth-Enter' class='non-auth' onclick=\"UserAuthEnter('Авторизация', '".$URL."');\">Войти</div>";
        ...
        if ((int)$_SESSION['userid'] == 0 && preg_match(
            ...
            $data = DB(
              "SELECT `id`,`nick`,`avatar`,`stat`,`role` FROM `_users` WHERE (`id`='".(int)$_SESSION['userid']."') LIMIT 1"
            );
            if ($data["total"] == 1) {
                @mysql_data_seek($data["result"], 0);
                $GLOBAL["USER"] = @mysql_fetch_array($data["result"]);
                if ($GLOBAL["USER"]["stat"] != 1) {
                ...
                ### Трекер комментариев
                $res = DB(
                  "SELECT `pid` FROM `_tracker` WHERE (`uid`='".(int)$_SESSION['userid']."' && `stat`='1')"
                );
                $ups = $res["total"];
                $upslast = $res["total"];
                if ((int)$ups > 0) {
                ...
                ### Аватар
                if (is_file(
                    $_SERVER['DOCUMENT_ROOT']."/".$GLOBAL["USER"]["avatar"]
                ...
                if ($_SESSION['updated'] == 0) {
                    $data = DB(
                      "UPDATE `_users` SET `lasttime`='".time(
                      )."' WHERE (`id`='".(int)$_SESSION['userid']."') LIMIT 1"
                    );
                    $_SESSION['updated'] = 1;
                }
            } else {
                $_SESSION['userid'] = 0;
                @header("Location: /users/lostid/");
                exit();
            }
        }
    }
