...
if ($HTTPREFERER == $_SERVER['HTTP_HOST']) {
    ...
    if ($pid != 0) {
        $data = DB(
          "SELECT `id` FROM `_likes` WHERE (`link`='_comments' && `pid`='".$pid."' && `ip`='".$ip."' && `data`>'".(time(
            ) - 1 * 24 * 60 * 60)."')"
        );
        $user = (int)$data["total"];
        ...
        if ($user == 0) {
            ...
            DB(
              ...
              )."','".$ip."','_comments','".$pid."','".$qid."')"
            );
            if ($qid == 0) {
                DB(
                  "UPDATE `_comments` set `dislikes`=`dislikes`+1 WHERE (`id`='".$pid."') LIMIT 1"
                );
            }
            if ($qid == 1) {
                DB(
                  "UPDATE `_comments` set `likes`=`likes`+1 WHERE (`id`='".$pid."') LIMIT 1"
                );
            }
        }
        # get likes and dislikes
        $data = DB(
          "SELECT `likes`,`dislikes` FROM `_comments` WHERE (`id`='".$pid."') LIMIT 1"
        );
