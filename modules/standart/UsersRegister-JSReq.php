...
if ($HTTPREFERER == $_SERVER['HTTP_HOST']) {
    ...
    if ($ll != $R["l"]) {
        ...
        if ($q["total"] == 0) {
            DB(
              "INSERT INTO `_users` (`login`, `nick`, `pass`, `role`, `ip`, `mail`, `stat`, `created`, `lasttime`) VALUES ('".$ll."', '".$nn."', '".$pp."', '0', '".$_SERVER['REMOTE_ADDR']."', '".$ee."', '1', '".time(
              )."', '".time()."')"
            );
            $_SESSION['userid'] = DBL();
            $_SESSION['userfrom'] = "";
            $_SESSION['userrole'] = 1;
            $result["Text"] = "Поздравляем с регистрацией на сайте...";
