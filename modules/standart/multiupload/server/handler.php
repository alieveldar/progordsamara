...
if ($_SESSION['userid'] != '') {
    ...
    if (!is_dir($ROOT.'/userfiles/temp')) {
        mkdir($ROOT.'/userfiles/temp', 0777);
    }
    ...
    if ($result['success']) {
        ...
        foreach ($GLOBAL['AutoPicPaths'] as $path => $size) {
            ...
            if ($path == "picpreview") {
                resize(
                  $ROOT."/userfiles/temp/".$picname,
                  $ROOT."/userfiles/".$path."/".$picname,
                  $sw,
                  $sh
                );
            } else {
        ...
        if ($_REQUEST['widget_pic']) {
            $res = DB(
              "INSERT INTO `".$table."` (`pid`,`$field`,`link`, `point`) VALUES ($pid, '$picname', '$link', '$point')"
            );
            $rate = DBL();
            ...
            $res = DB(
              "INSERT INTO `".$table."` (`pid`, `pic`, `data`) VALUES ($pid, '$picname', '".time(
              )."')"
            );
            $rate = DBL();
            DB("UPDATE `".$table."` SET `rate`=$rate WHERE `id`=$rate");
            DB(
              "UPDATE `".$table."` SET `uid`=".$_SESSION['userid']." WHERE `id`=$rate"
            );
        }
    }
