...
if ($HTTPREFERER == $_SERVER['SERVER_NAME']) {
    ...
    if ($R["action"] == 'spam') {
        ...
        if ($uid) {
            ...
            if (function_exists('MailSend') && MailSend(
                $VARS["adminmail"],
                $subject,
                $body,
                $VARS["sitemail"]
        ...
        if ($UserSetsSite[3] == 1) {
            ...
            if ($uid == 0 && $UserSetsSite[4] == 0 && $error == 0) {
                ...
                $result = array(
                  "Code" => 0,
                  "Text" => "Для добавления комментария необходимо авторизоваться",
                  "Class" => "ErrorDiv",
                  "Comment" => "",
                );
            }
            ...
            if ($cnt >= 2 && $error == 0) {
                ...
                $result = array(
                  "Code" => 10,
                  "Text" => "Спасибо за SPAM! [filter=$cnt]",
                  "Class" => "ErrorDiv",
                  "Comment" => "",
                );
                DB(
                  "UPDATE `_users` SET `stat`='0' WHERE (`id`='".$uid."') LIMIT 1"
                );
                $_SESSION["userid"] = 0;
                $_SESSION['userrole'] = 0;
            }
            if ($l == "" && $error == 0) {
                ...
                $result = array(
                  "Code" => 0,
                  "Text" => "Не определен раздел комментария",
                  "Class" => "ErrorDiv",
                  "Comment" => "",
                );
            }
            if ($i == 0 && $error == 0) {
                ...
                $result = array(
                  "Code" => 0,
                  "Text" => "Не определен материал комментария",
                  "Class" => "ErrorDiv",
                  "Comment" => "",
                );
            }
            if ($uid == 0 && $UserSetsSite[4] == 1 && $UserSetsSite[5] == 1 && $c != $p && $error == 0) {
                ...
                $result = array(
                  "Code" => 0,
                  "Text" => "Введен неверный защитный код!",
                  "Class" => "ErrorDiv",
                  "Comment" => "",
                );
            }
            if ($error == 0) {
                $d = DB(
                  "SELECT `comments` FROM `".$l."_lenta` WHERE (`id`='".$i."') LIMIT 1"
                );
                if ($d["total"] != 1) {
                    $error = 1;
                    $result = array(
                      "Code" => 0,
                      "Text" => "Материал отсутствует на сайте",
                      "Class" => "ErrorDiv",
                      "Comment" => "",
                    );
                } else {
                    ...
                    if ($ar["comments"] != 0 && $_SESSION["userrole"] < 1) {
                        ...
                        $result = array(
                          "Code" => 0,
                          "Text" => "В данном материалы запрещены комментарии",
                          "Class" => "ErrorDiv",
                          "Comment" => '',
                        );
                    }
        ...
        if ($error == 0 && $UserSetsSite[3] == 1) {
            ...
            DB(
              "INSERT INTO `_comments` (`link`,`pid`,`uid`,`uname`,`from`,`data`,`toid`,`text`,`ip`,`referer`,`vkid`)
		VALUES ('".$l."', '".$i."', '".$uid."', '".$n."', '".$ufrom."', '".time(
              )."', '".$toid."', '".$t."', '".$GLOBAL["ip"]."', '".$_SESSION['userreferer']."', '$vki');"
            );
            $last = DBL();
            DB(
              "UPDATE `".$l."_lenta` set `comcount`=`comcount`+1 WHERE (`id`='".$i."') LIMIT 1"
            );
            DB(
              "UPDATE `".$l."_lenta` set `update`='".time(
              )."', `clast`='".$last."' WHERE (`id`='".$i."') LIMIT 1"
            ); /* Для форума */
            ### обновляем трекер
            if ($uid != 0) {
                DB(
                  "INSERT INTO `_tracker` (`uid`,`link`,`pid`,`data`,`stat`) VALUES ('".$uid."','".$l."','".$i."','".time(
                  )."', '0') ON DUPLICATE KEY UPDATE `data`='".time()."'"
                );
            }
            DB(
              "UPDATE `_tracker` SET `stat`='1', `data`='".time(
              )."' WHERE (`link`='".$l."' AND `pid`='".$i."' AND `uid`<>'$uid')"
            );
            ### Вложения
            if ($UserSetsSite[7] == 1) {
                foreach ($fl as $k => $pic) {
                    if (is_file(
                      $_SERVER['DOCUMENT_ROOT']."/userfiles/temp/".$pic
                    )) {
                        @rename(
                          $_SERVER['DOCUMENT_ROOT']."/userfiles/temp/".$pic,
                        ...
                        DB(
                          "INSERT INTO `_commentf` (`pid`,`uid`,`pic`) VALUES ('".$last."','".$uid."','".$pic."');"
                        );
                    }
                }
            }
            if ($UserSetsSite[7] == 1 && $toid) {
                $f = DB(
                  "SELECT `pic`, `pid` FROM `_commentf` WHERE (`pid`=".$toid.") ORDER BY `id` ASC"
                );
                $fl2 = array();
                for ($j = 0; $j < $f["total"]; $j++) {
            ...
            for ($j = 0; $j < $data["total"]; $j++) {
                ...
                if ($com["uid"] == 0) {
                    ...
                    if (is_file(
                        ...
                        if (in_array($RealHost, array('nsk.progorodsamara.ru', 'nsk.samara.cc'))){
                            $avatar = "<a target='_blank' href='/users/view/".$com["uid"]."/'><img src='/userfiles/avatar/no_photo_nsk.png'></a>";
                        } else {
                ...
                /* Кнопка ответить */
                $text .= $C5."<div class='CommentAnswer' id='CommentAnswer-".$com["id"]."'></div></i></b>";
                $text .= $C."</div>";

            }
            @unlink(
              $_SERVER['DOCUMENT_ROOT']."/cache/user_comments/".$l.".".$i.".cache"
            ); // Удаляем кеш комментариев
            $result = array(
              "Code" => 1,
              "Text" => "Ваш комментарий успешно добавлен! [$cnt]",
              "Class" => "SuccessDiv",
              "Comment" => $text,
            );
        }
...
{
    @file_put_contents(
      $_SERVER['DOCUMENT_ROOT']."/modules/standart/debug-comment-add.txt",
      $t
    );
}
