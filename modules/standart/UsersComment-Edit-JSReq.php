...
if ($HTTPREFERER == $_SERVER['SERVER_NAME']) {
    ...
    if ($R["action"] == "form") {
        $result = GetFormComments((int)$R["id"]);
    } else {
        $t = $R["t"];
        $t = str_replace("'", "&#039;", $t);
        ...
        if ($UserSetsSite[3] == 1) {
            ...
            if (checkAttrs($t)) {
                ...
                $result = array(
                  "Code" => 0,
                  "Text" => "Запрещается использовать теги кроме &lt;b&gt;, &lt;i&gt;, &lt;u&gt;, &lt;p&gt;",
                  "Class" => "ErrorDiv",
                  "Comment" => "",
                );
            }
            ...
            if ($error == 0) {
                ...
                $d = DB(
                  "UPDATE `_comments` SET `text`='".$t."' WHERE (`id`='".(int)$R["id"]."' $where)"
                );
                $result = array(
                  "Code" => 1,
                ...
                $data = DB(
                  "SELECT `link`, `pid` FROM `_comments` WHERE (`id`='".(int)$R["id"]."')"
                );
                @mysql_data_seek($data["result"], 0);
                $com = @mysql_fetch_array($data["result"]);
                $file = "user_comments-".$com['link'].".".$com['pid'];
                ClearCache($file);
            }
        }
    }
...
function GetFormComments($id)
{
    ...
    $where = $_SESSION["userrole"] > 2 ? "" : "AND `uid`=".$_SESSION['userid'];
    $data = DB(
      "SELECT `text` FROM `_comments` WHERE (`id`='".(int)$id."' $where)"
    );
    if ($data["total"]) {
        @mysql_data_seek($data["result"], 0);
        $com = @mysql_fetch_array($data["result"]);
