...
$CSSmodules = array(/* "Стандартный Pro.CMS"				=> "/template/standart/standart.css", */
);
...
if (is_file("modules/mobile.StaticBlocks.php")) {
    require("modules/mobile.StaticBlocks.php");
    $GLOBAL["log"] .= "<i>Подключение PHP</i>: общий модуль &laquo;mobile.StaticBlocks.php&raquo; подключен<hr>";
}
if ($IsIndex == 1) {
...
if ($data["total"] == 0) {
    ...
    if ($node["module"] == "") {
        ...
        if (is_file(
          "modules/".$node["module"]."/mobile/".$node["module"].".php"
        )) {
            /* PHP */
            @header("HTTP/1.0 200 Ok");
            $Robots = '<meta name="robots" content="index, follow" />';
            @require("modules/".$node["module"]."/mobile/".$node["module"].".php");
            /* JS */
            if (is_file(
              "modules/".$node["module"]."/mobile/".$node["module"].".js"
            )) {
                $JSmodules[$node["name"]] = "/modules/".$node["module"]."/mobile/".$node["module"].".js";
            }
            /* CSS */
            if (is_file(
              "modules/".$node["module"]."/mobile/".$node["module"].".css"
            )) {
                $CSSmodules[$node["name"]] = "/modules/".$node["module"]."/mobile/".$node["module"].".css";
            }
    ...
    if (is_file(
        "modules/page_mods/mobile/".$node["link"].".php"
      ) && $Page404 != 1) {
        @require("modules/page_mods/mobile/".$node["link"].".php");
        $GLOBAL["log"] .= "<i>Подключение PHP</i>: модуль &laquo;modules/page_mods/mobile/".$node["link"].".php&raquo; подключен<hr>";
    }
    if (is_file(
        "modules/page_mods/mobile/".$node["link"].".js"
      ) && $Page404 != 1) {
        $JSmodules["modules/page_mods/mobile/".$node["link"].".js"] = "/modules/page_mods/mobile/".$node["link"].".js";
    }
    if (is_file(
        "modules/page_mods/mobile/".$node["link"].".css"
      ) && $Page404 != 1) {
        $CSSmodules["modules/page_mods/mobile/".$node["link"].".css"] = "/modules/page_mods/mobile/".$node["link"].".css";
    }
}
...
if ($Page["Caption"] != "" && $Page404 == 0) {
    $Page["Caption"] = "<h1>".$Page["Caption"]."</h1>";
}
if ($Page["Caption"] != "" && $Page404 == 1) {
    $Page["Caption"] = "<h1 align='center'>".$Page["Caption"]."</h1>";
}
...
echo '<meta name="description" content=\''.trim(
    $Page["Description"],
    ","
  ).'\' />'.$r;
echo '<meta name="apple-itunes-app" content="app-id=1087923563">';
echo '<meta name="google-play-app" content="app-id=air.ru.prosamara.news">';
...
echo '</head>'.$r.'<body>'.$r;
echo "<div class='Banner' id='Banner-30-1' style='text-align:center;'></div>";
echo $DesignHtml.$r;
echo "";
echo '<script type="text/javascript">
