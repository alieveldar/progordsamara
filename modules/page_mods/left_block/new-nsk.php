<?
$file = "_leftblock-SMddefault";
if (RetCache($file, "cacheblock") == "true") {
    list($Page["LeftContent"], $cap) = GetCache($file, 0);
} else {
    list($Page["LeftContent"], $cap) = CreateLeftBlock();
    SetCache($file, $Page["LeftContent"], "", "cacheblock");
}

function CreateLeftBlock()
{
    global $Domains, $SubDomain, $GLOBAL, $C20, $C10, $C25, $C, $used;
    $text = '';
    $src = "";
    $advs = array();
    $news = array();
    ...
    /*TV*/
    $q = "SELECT `[table]`.`id`, `[table]`.`name`, `[table]`.`tavto`, `[table]`.`lid`, `[table]`.`data`, `[table]`.`comcount`, `[table]`.`pic`, '[link]' as `link` FROM `[table]`
WHERE (`[table]`.`stat`='1' && `[table]`.`onind`=1 [used]
AND (main_column_pronsk = 1 OR left_column_pronsk = 1 OR main_column_pronsk_both = 1 OR left_column_pronsk_both = 1)
    ...
    for ($i = 0; $i < $data["total"]; $i++): @mysql_data_seek(
      ...
      $i
        ...
        $used[$ar["link"]][] = $ar["id"];
        $text .= "<div class='ONew'><a href='/".$ar["link"]."/view/".$ar["id"]."'>";
        if ($i == 0 && $ar["pic"] != "") {
    ...
    $q = "SELECT `[table]`.`id`, `[table]`.`name`, `[table]`.`lid`, `[table]`.`tavto`,`[table]`.`data`, `[table]`.`pic`, '[link]' as `link` FROM `[table]` WHERE (`[table]`.`stat`='1' && `[table]`.`data`<'".(time(
        ...
        ) - 7 * 24 * 60 * 60)."' && (`[table]`.`commerce_column_pronsk_both`=1 || `[table]`.`spromo`=1) [used])";
    ...
    /*NEWS*/
    $q = "SELECT `[table]`.`id`, `[table]`.`name`, `[table]`.`lid`, `[table]`.`tavto`,`[table]`.`data`, `[table]`.`comcount`, `[table]`.`pic`, '[link]' as `link` FROM `[table]`
WHERE (`[table]`.`stat`='1' [used]
AND (main_column_pronsk = 1 OR left_column_pronsk = 1 OR main_column_pronsk_both = 1 OR left_column_pronsk_both = 1))";
