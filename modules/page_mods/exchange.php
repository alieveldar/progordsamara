...
{
    $C = "";
    $txt = @file_get_contents("http://dengi.63.ru/exchange/exchange.html");
    $txt = iconv("windows-1251", "utf-8", $txt);
    $tm = explode('Версия	для печати', $txt);
    $text = $tm[1];
    $text = str_replace(array('<br />', '<br>'), "", $text);
    $text = strip_tags($text, "<table><tr><th><td><font><b><span>");
