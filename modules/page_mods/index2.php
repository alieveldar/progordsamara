...
function CENTER()
{
    ...
    $q = "SELECT `[table]`.`id`, `[table]`.`name`, `[table]`.`lid`, `[table]`.`tavto`,`[table]`.`data`, `[table]`.`comcount`, `[table]`.`pic`, '[link]' as `link` FROM `[table]` WHERE (`[table]`.`stat`='1' && (`[table]`.`redak`=1 OR `[table]`.`main_column_pronsk_both` = 1) [used] AND (main_column_pronsk <> 1 AND left_column_pronsk <> 1 AND commerce_column_pronsk_both <> 1))";
...
function RIGHT()
{
    ...
    $text .= '<!--В head сайта один раз подключите библиотеку-->
<!--AdFox START-->
<!--Orion_sites-->
<!--Площадка: progorodsamara.ru / * / *-->
