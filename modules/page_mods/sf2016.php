...
$data = DB("SELECT * FROM `sport_lenta` WHERE (`id`='148147')");
...
$endq = "ORDER BY `data` DESC LIMIT 7";
...
$data = DB(
  "SELECT * FROM `_widget_pics` WHERE (`pid`='148147' && `link`='sport') order by `rate` asc"
);
$bomb .= "<div>";
$numb = 1;
for ($i = 0; $i < $data["total"]; $i++) {
    ...
    $ar = @mysql_fetch_array($data["result"]);
    if ($ar["name"] != "") {
        $bomb .= "</div><cl></cl><h2>".$ar["name"]."</h2><div>";
        $numb = 1;
    }
    ...
    $bomb .= "<div class='name'>".$ar["author"]."<br /><b>".strip_tags(
        $ar["text"]
      )."</b></div>";
    $bomb .= "</div>";
    if ($numb % 3 == 0) {
...
$bomb .= "</div>";
$Page["Content"] .= "<div class='bomb'>".$bomb."</div>";
