...
switch ($start) {
    ...
    case 'letter' :
        ...
        //кеш категорий
        if (RetCache($catsListCacheFile) == "true") {
            list($catsBlockText, $captionText) = GetCache(
              $catsListCacheFile,
              0
            );
        } else {
            ...
            $rubricList = getRubricListByLetter(
              $letter,
              $offset,
              $itemsPerPage
            );
            ...
            $rubricListLetterBlock = displayRubricListBlock(
              $rubricList,
              $dir[0],
              true
            );
            $rubricListPagerBlock = Pager2(
              $pg,
              $itemsPerPage,
              ceil($rubricCount / $itemsPerPage),
        ...
        if (RetCache($itemCacheFile) == "true") {
            ...
            if ($rubricItem == null) {
                ...
                if ($GLOBAL["USER"]["role"] > 1) {
                    $text = $C10."<div id='AdminEditItem'><a href='".$GLOBAL["mdomain"]."/admin/?cat=".$link."_edit&id=".(int)$dir[2]."'>Редактировать</a></div>".$C15.InitImageMaster(
                        $cap,
                        $new["pic"]
                      ).$C15.$text;
                }
    ...
    case 'cat' :
        ...
        $rubricList = getRubricListByCategoryId(
          $categoryId,
          $offset,
          $itemsPerPage
        );
...
function displayRubricListBlock(
  array $rubricList,
  $baseUrl,
  $displayCat = false
) {
...
function displayRubricItem($rubricItem, $baseUrl, $forums)
{
    ...
    switch ($rubricItem['cat']) {
        case 1:
            ...
            if ($rubricItem['birth_date'] !== null) {
                $birthDateString = getFormattedWikiDate(
                  $rubricItem['birth_date']
                );
                $tableAdditionalText .=
                  "<tr>
                    <td>Дата рождения</td>
            ...
            if ($rubricItem['death_date'] !== null) {
                $deathDateString = getFormattedWikiDate(
                  $rubricItem['death_date']
                );
                $tableAdditionalText .=
                  "<tr>
                    <td>Дата смерти</td>
        ...
        case 2:
            if ($rubricItem['start_date'] !== null) {
                $startDateString = getFormattedWikiDate(
                  $rubricItem['start_date']
                );
                $tableAdditionalText .=
                  "<tr>
                    <td>Дата начала</td>
        ...
        case 3:
            ...
            if ($rubricItem['release_date'] !== null) {
                $releaseDateString = getFormattedWikiDate(
                  $rubricItem['release_date']
                );
                $tableAdditionalText .=
                  "<tr>
        ...
        case 4:
            ...
            break;
        case 5:
            $acceptDateString = getFormattedWikiDate(
              $rubricItem['accept_date']
            );
            $tableAdditionalText .=
              "<tr>
                    <td>Автор</td>
        ...
        case 6:
            ...
            if ($rubricItem['foundation_date'] !== null) {
                $foundationDateString = getFormattedWikiDate(
                  $rubricItem['foundation_date']
                );
                $tableAdditionalText .=
                  "<tr>
                    <td>Дата создания</td>
            ...
            if ($rubricItem['decay_date'] !== null) {
                $decayDateString = getFormattedWikiDate(
                  $rubricItem['decay_date']
                );
                $tableAdditionalText .=
                  "<tr>
                    <td>Дата распада</td>
        ...
        case 7:
            ...
            if ($rubricItem['invention_date'] !== null) {
                $inventionDateString = getFormattedWikiDate(
                  $rubricItem['invention_date']
                );
                $tableAdditionalText .=
                  "<tr>
    ...
    if (isset($eventMap)) {
        if ($eventMap['maps']) {
            ...
            $event .= '<script type="text/javascript">initMap(['.$eventMap['id'].', "'.htmlspecialchars(
                $eventMap['name']
              ).'", "'.$eventMap['maps'].'", "'.($eventMap['icon'] ? '/userfiles/mapicon/'.$eventMap['icon'] : '').'"]);</script>';
            ...
            if ($eventMap['data']) {
                ...
                $event_first_day = getdate(
                  mktime(
                    ...
                    0,
                    0,
                    date('m', $eventMap['data']),
                    1,
                    date('Y', $eventMap['data'])
                  )
                );
