...
if (!$ar["stat"]) {
    ...
    if (isset($_SESSION['Data']["what"])) {
        $P = $_SESSION['Data'];
        SD();
        @header("location: /".$dir[0]."/search/".$P["what"]."/1/".$P["where"]);
        exit();
    }
        ...
        if (!empty($xml) and $xml->response_code == 200) {
            ...
            if ($xml->advertising) {
                ...
                foreach ($xml->advertising->advertisement as $adv) {
                    ...
                    if ($i < sizeof($xml->advertising->advertisement)) {
                        $Page['Content'] .= "<div class='C5'></div><div class='CBG'></div>";
                    }
                    $i++;
                }
            ...
            while ($i <= ($count_str - 1)):
                ...
                if ($xml->result->filial[$i]->firm_group->count > 1) {
                    $Page['Content'] .= ", <a href='/".$dir[0]."/filials/".$xml->result->filial[$i]->firm_group->id."' class='filials'> Филиалов: ".$xml->result->filial[$i]->firm_group->count."</a>";
                }
                $Page['Content'] .= "</div><div class='C'></div></div><div class='C10'></div>";
                $arr_na_kartu['arr_data'][$i]['name'] = "".$xml->result->filial[$i]->name;
                $arr_na_kartu['arr_data'][$i]['adress'] = "".$xml->result->filial[$i]->adress;
                $arr_na_kartu['arr_data'][$i]['lon'] = "".$xml->result->filial[$i]->lon;
                $arr_na_kartu['arr_data'][$i]['lat'] = "".$xml->result->filial[$i]->lat;
                $i++;
            endwhile;
            $page_total = $xml->result['total'];
            $Page['Content'] .= Pager2(
              $page,
              25,
              ceil($page_total / 25),
              $dir[0].'/search/'.$what.'/[page]/'.$where
            );
            ...
            $i = 0;
            while ($i <= ($count_str - 1)): {
                if (($arr_na_kartu['arr_data'][$i]['lon'] <> '') or ($arr_na_kartu['arr_data'][$i]['lat'] <> '')) {
            ...
            $Page['Content'] .= "По Вашему запросу ничего не найдено";
        }
    }
        ...
        if (!empty($xml_rubrics)) {
            ...
            while ($i <= ($count_str - 1)): $Page['Content'] .= "<div class='WhiteBlock MainItem1Gis'>";
                ...
                if (!empty($xml_rubrics_add)) {
                    ...
                    $i1 = 0;
                    while ($i1 <= 2):
                        if ($xml_rubrics_add->result->rubric[$i1]->id > 0) {
                            $Page['Content'] .= "<a href='/".$dir[0]."/podrubrik/".$xml_rubrics_add->result->rubric[$i1]->name."'>".$xml_rubrics_add->result->rubric[$i1]->name."</a>, ";
                        }
                        $i1++;
                    endwhile;
...
// #######################################################################################################################################
    ...
    if ($dir[1] == 'rubrik') {
        $rub = $dir[2];
        $zapros = 'http://catalog.api.2gis.ru/rubricator?where='.$sets[0].'&version=1.3&id='.$rub.'&output=xml&key='.$sets[1];
        $xml = simplexml_load_file($zapros);
...
// #######################################################################################################################################
    ...
    if ($dir[1] == 'podrubrik') {
        ...
        $zapros = 'http://catalog.api.2gis.ru/search?what='.$podrub.'&where='.$sets[0].'&page='.$page.'&pagesize=25&key='.$sets[1].'&version=1.3&sort=relevance&output=xml';
        $xml = simplexml_load_file($zapros);
        ...
        if (!empty($xml) and $xml->response_code == 200) {
            $Page['Content'] .= "<h2>Список компаний раздела:</h2><div id='myMapId'></div><div class='C15'></div>";
            $page_total = $xml->result['total'];
            $count_str = count($xml->result->filial);
            $i = 0;
            while ($i <= ($count_str - 1)): $num = ($i + 1) + (($page - 1) * 25);
                ...
                $arr_na_kartu['arr_data'][$i]['lat'] = "".$xml->result->filial[$i]->lat;
                $i++;
                $Page['Content'] .= "</div><div class='C'></div></div><div class='C10'></div>"; endwhile;
            $Page['Content'] .= "<div class='C10'></div>";
            $Page['Content'] .= Pager2(
              $page,
              25,
              ceil($page_total / 25),
              $dir[0]."/podrubrik/".$podrub."/[page]"
            );
            $Page['Content'] .= '<script type="text/javascript">DG.autoload(function() { var myMap = new DG.Map("myMapId"); myMap.setCenter(new DG.GeoPoint('.$arr_na_kartu['arr_data'][0]['lon'].','.$arr_na_kartu['arr_data'][0]['lat'].'), 12); myMap.controls.add(new DG.Controls.Zoom());';
            $i = 0;
            while ($i <= ($count_str - 1)): {
...
// #######################################################################################################################################
    ...
    if ($dir[1] == 'info') {
        $info = urldecode($dir[2]);
        $has = urldecode($dir[3]);
        $page = 1;
        $zapros = "http://catalog.api.2gis.ru/profile?id=".$info."&output=xml&key=".$sets[1]."&version=1.3&hash=".$has;
        $xml = simplexml_load_file($zapros);
        ...
        if (!empty($xml)) {
            $res = @file_get_contents($xml->profile->register_bc_url);
            $Page['Content'] .= "<h2>".$xml->profile->name."</h2>";  ### РЕГИСТРАЦИЯ
            if ($xml->profile->article) {
                $Page['Content'] .= "<div class='MainItem1Gis' style='font-size:12px; font-weight:normal;'>".$xml->profile->article."</div><div class='C15'></div>";
            }
            if ($xml->profile->lon && $xml->profile->lat) {
                $Page['Content'] .= "<div id='myMapId'></div><div class='C15'></div>";
            }
            $Page['Content'] .= "<div class='MainItem1Gis'>Адрес: ".$xml->profile->city_name.", ".$xml->profile->address."<div class='C5'></div>";
            $tel = 0;
            while ($tel <= 5):
                if ($xml->profile->contacts->group->contact[$tel]['type'] == 'phone') {
                    $Page['Content'] .= "тел.: ".$xml->profile->contacts->group->contact[$tel]->value."<div class='C5'></div>";
                }
                if ($xml->profile->contacts->group->contact[$tel]['type'] == 'fax') {
                    $Page['Content'] .= "факс.: ".$xml->profile->contacts->group->contact[$tel]->value."<div class='C5'></div>";
                }
                if ($xml->profile->contacts->group->contact[$tel]['type'] == 'website') {
                    $Page['Content'] .= "<noindex>Сайт: <a target='_blank  ' href='".$xml->profile->contacts->group->contact[$tel]->value."' rel='nofollow'>".$xml->profile->contacts->group->contact[$tel]->alias."</a><noindex><div class='C5'></div>";
                }
                if ($xml->profile->contacts->group->contact[$tel]['type'] == 'email') {
                    $Page['Content'] .= "<noindex>E-mail: <a href='mailto:".$xml->profile->contacts->group->contact[$tel]->value."' rel='nofollow'>".$xml->profile->contacts->group->contact[$tel]->value."</a><noindex><div class='C5'></div>";
                }
            ...
            $Page['Content'] .= "<td>".$xml->profile->schedule->day[0]->working_hours[0]['from']." - ".$xml->profile->schedule->day[0]->working_hours[0]['to'];
            if ($xml->profile->schedule->day[0]->working_hours[1]['from']) {
                $Page['Content'] .= "<br />".$xml->profile->schedule->day[0]->working_hours[1]['from']." - ".$xml->profile->schedule->day[0]->working_hours[1]['to'];
            }
            $Page['Content'] .= "</td>";

            $Page['Content'] .= "<td>".$xml->profile->schedule->day[1]->working_hours[0]['from']." - ".$xml->profile->schedule->day[1]->working_hours[0]['to'];
            if ($xml->profile->schedule->day[1]->working_hours[1]['from']) {
                $Page['Content'] .= "<br />".$xml->profile->schedule->day[1]->working_hours[1]['from']." - ".$xml->profile->schedule->day[1]->working_hours[1]['to'];
            }
            $Page['Content'] .= "</td>";

            $Page['Content'] .= "<td>".$xml->profile->schedule->day[2]->working_hours[0]['from']." - ".$xml->profile->schedule->day[2]->working_hours[0]['to'];
            if ($xml->profile->schedule->day[2]->working_hours[1]['from']) {
                $Page['Content'] .= "<br />".$xml->profile->schedule->day[2]->working_hours[1]['from']." - ".$xml->profile->schedule->day[2]->working_hours[1]['to'];
            }
            $Page['Content'] .= "</td>";

            $Page['Content'] .= "<td>".$xml->profile->schedule->day[3]->working_hours[0]['from']." - ".$xml->profile->schedule->day[3]->working_hours[0]['to'];
            if ($xml->profile->schedule->day[3]->working_hours[1]['from']) {
                $Page['Content'] .= "<br />".$xml->profile->schedule->day[3]->working_hours[1]['from']." - ".$xml->profile->schedule->day[3]->working_hours[1]['to'];
            }
            $Page['Content'] .= "</td>";

            $Page['Content'] .= "<td>".$xml->profile->schedule->day[4]->working_hours[0]['from']." - ".$xml->profile->schedule->day[4]->working_hours[0]['to'];
            if ($xml->profile->schedule->day[4]->working_hours[1]['from']) {
                $Page['Content'] .= "<br />".$xml->profile->schedule->day[4]->working_hours[1]['from']." - ".$xml->profile->schedule->day[4]->working_hours[1]['to'];
            }
            $Page['Content'] .= "</td>";

            if ($xml->profile->schedule->day[5]->working_hours['from'] <> '') {
                $Page['Content'] .= "<td>".$xml->profile->schedule->day[5]->working_hours['from']." - ".$xml->profile->schedule->day[5]->working_hours['to']."</td>";
            } else {
                $Page['Content'] .= "<td>выходной</td>";
            }
            if ($xml->profile->schedule->day[6]->working_hours['from'] <> '') {
                $Page['Content'] .= "<td>".$xml->profile->schedule->day[6]->working_hours['from']." - ".$xml->profile->schedule->day[6]->working_hours['to']."</td>";
            } else {
                $Page['Content'] .= "<td>выходной</td>";
            }

            $Page['Content'] .= "</tr></table>";
            if ($xml->profile->comment) {
                $Page['Content'] .= "<div class='WhiteBlock MainItem1Gis' style='font-size:12px; font-weight:normal;'>".$xml->profile->comment."</div><div class='C15'></div>";
            }
            ...
            if ($xml->profile->fas_warning) {
                $Page['Content'] .= "<div class='CenterText'>Предупереждение ФАС: ".$xml->profile->fas_warning."</div>";
            }
        ...
        $zapros_other = "http://catalog.api.2gis.ru/search?what=".$xml->profile->rubrics->rubric."&where=".$sets[0]."&pagesize=25&key=".$sets[1]."&version=1.3&sort=relevance&output=xml";
        $xml_other = simplexml_load_file($zapros_other);
        if (!empty($xml_other) and $xml_other->response_code == 200) {
            $Page['Content'] .= "<p>&nbsp;</p><h2>Другие компании раздела:</h2>";
            $count_str = count(
              $xml_other->result->filial
            ) > 3 ? 3 : $xml_other->result->filial;
            $num = 1;
            while ($num <= $count_str):
                $item = $xml_other->result->filial[rand(
                  0,
                  count($xml_other->result->filial) - 1
                )];
                if ($item->id == $info) {
                    continue;
                }
                ...
                if ($item->firm_group->count > 1) {
                    $Page['Content'] .= ", <a href='/".$dir[0]."/filials/".$item->firm_group->id."' class='filials'> Филиалов: ".$item->firm_group->count."</a>";
                }
                $num++;
                $Page['Content'] .= "</div><div class='C'></div></div><div class='C10'></div>"; endwhile;
        }
...
// #######################################################################################################################################
    ...
    if ($dir[1] == "filials") {
        $filials = urldecode($dir[2]);
        $hash = urldecode($dir[3]);
        $arr_na_kartu = array(
          "arr_data" => array(
            "name" => "",
            "addres" => "",
        ...
        $Page['Content'] .= "<div id='myMapId'></div><div class='C15'></div>";
        $zapros = 'http://catalog.api.2gis.ru/firmsByFilialId?firmid='.$filials.'&where='.$sets[0].'&page=1&key='.$sets[1].'&version=1.3&sort=relevance&output=xml';
        $xml = @simplexml_load_file($zapros);
