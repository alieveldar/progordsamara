...
if ($start == "view") {
    ...
    $data = DB(
      "SELECT `$table`.`id`, `$table`.`name`, `$table`.`text`, `$table`.`comments`, `$table`.`votingend`, `$table`.`pic`, `$table`.`elemsstyle`, `$table`.`winnerscount`
...
function GetConcursId($item)
{
    ...
    $text = $item['text'];
    if ($item['pic']) {
        $text .= '<div class="CenterText"><a href="/userfiles/demots/'.$item['pic'].'.jpg" rel="prettyPhoto[gallery]"><img src="/userfiles/demots/'.$item['pic'].'.jpg" width="400" border="0" /></a></div>';
    }
    ...
    if ($GLOBAL["now"] > $item["votingend"]) {
        ...
        $data = DB(
          "SELECT `".$table2."`.*, COUNT(`".$table3."`.`id`) as `cnt` FROM `".$table2."` LEFT JOIN `".$table3."` ON `".$table3."`.`vid`=`".$table2."`.`id` WHERE (`".$table2."`.`link`='".($dir[0])."' AND `".$table2."`.`pid`=".(int)$dir[2]." AND `".$table2."`.`stat`=1) GROUP BY 1 ORDER BY `".$table2."`.`rate`"
        );
        $q = DB(
          "SELECT `id` FROM `$table3` WHERE (`ip`='".$GLOBAL["ip"]."' AND `pid`=".(int)$dir[2]." AND `link`='$link' AND `data`>".$GLOBAL["tonight"].")"
        );
