...
if ($HTTPREFERER == $_SERVER['SERVER_NAME']) {
    ...
    $q = DB(
      "SELECT `id` FROM `$table3` WHERE ((`ip`='".$GLOBAL["ip"]."' $or) AND `pid`=$pid AND `link`='$link' AND `data`>".$GLOBAL["tonight"].")"
    );
    $q2 = DB("SELECT `voting` FROM `".$table."` WHERE (`id`=".$pid.") LIMIT 1");
    @mysql_data_seek($q2["result"], 0);
    $node = @mysql_fetch_array($q2["result"]);
    ...
    if ($q["total"] == 1) {
        ...
        $data = DB(
          "SELECT `".$table2."`.`id`, COUNT(`".$table3."`.`id`) as `cnt` FROM `".$table2."` LEFT JOIN `".$table3."` ON `".$table3."`.`vid`=`".$table2."`.`id` WHERE (`".$table2."`.`link`='".$link."' AND `".$table2."`.`pid`=".$pid." AND `".$table2."`.`stat`=1) GROUP BY 1"
        );
        $result["Votes"] = array();
