...
if ($HTTPREFERER == $_SERVER['SERVER_NAME']) {
    ...
    $result = array();
    $data = DB(
      "SELECT `voting`, `votingmode`, `votingend` FROM `".$table."` WHERE (`id`=".$pid.") LIMIT 1"
    );
    ...
    if ($GLOBAL["now"] < $item["votingend"] && $item["votingmode"] == 1) {
        $q = DB(
          "SELECT `id` FROM `$table3` WHERE ($where AND `pid`=".$pid." AND `link`='$link' AND `data`>".$GLOBAL["tonight"].")"
        );
    }
    ...
    if ($q["total"]) {
        ...
        if ($GLOBAL["now"] < $item["votingend"]) {
            DB(
              "INSERT INTO `$table3` (`link`,`vid`,`uid`,`data`,`ip`,`pid`) VALUES ('$link', $vid, ".$_SESSION['userid'].", '".$GLOBAL["now"]."', '".$GLOBAL["ip"]."', '$pid')"
            );

            $data = DB(
              "SELECT `".$table2."`.`id`, COUNT(`".$table3."`.`id`) as `cnt` FROM `".$table2."` LEFT JOIN `".$table3."` ON `".$table3."`.`vid`=`".$table2."`.`id` WHERE (`".$table2."`.`link`='".$link."' AND `".$table2."`.`pid`=".$pid." AND `".$table2."`.`stat`=1) GROUP BY 1 ORDER BY `".$table2."`.`rate` ASC"
            );
            $result["Votes"] = array();
            ...
            $result["Code"] = 1;
            if ($item["votingmode"] == 1) {
                $result["btnRemove"] = true;
            }
        }
    }
} else {
