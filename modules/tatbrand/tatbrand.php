...
if ($start == "view") {
    $where = $GLOBAL["USER"]["role"] == 0 ? "AND `stat`=1" : "";
    $data = DB(
      "SELECT `id`, `name`, `lid`, `pic`, `text`, `stat`, `voting`, `votingend`, `votingmode`, `winnerscount`, `comments`, `domain` FROM `".$table."` WHERE (`id`='".(int)$dir[2]."' $where) LIMIT 1"
    );
    if ($data["total"]) {
        ...
        if ($GLOBAL["now"] < $item["votingend"] && $item["votingmode"] == 1) {
            $q = DB(
              "SELECT `id` FROM `$table3` WHERE ($where AND `pid`=".$item['id']." AND `link`='$link' AND `data`>".$GLOBAL["tonight"].")"
            );
        }
        if ($GLOBAL["now"] > $item["votingend"] || ($item['voting'] == 1 && !$_SESSION['userid']) || $q["total"]) {
            $script = '<script type="text/javascript">$("#node'.$item['id'].'.votingCon .votingButton a").remove();</script>';
        }

        if ($GLOBAL["now"] < $item["votingend"] && $item['voting'] == 1 && !$_SESSION['userid']) {
            $script = '<script type="text/javascript">$("#node'.$item['id'].'.votingCon .votingButton a").remove(); $("#node'.$item['id'].'.votingCon .Info").html("Голосовать могут только зарегистрированные пользователи").show();</script>';
        } else {
            if ($GLOBAL["now"] < $item["votingend"] && $q["total"]) {
                $script = '<script type="text/javascript">$("#node'.$item['id'].'.votingCon .votingButton a").remove(); $("#node'.$item['id'].'.votingCon .Info").html("Спасибо, ваш голос учтён!").show();</script>';
            }
        }
    } else {
...
function GetConcursId($item)
{
    ...
    if ($item['lid']) {
        $text .= "<div class='ItemLid'>".$item['lid']."</div>".$C10;
    }
    $text .= $item['text'];
    ...
    $text .= $C20."<div class='Likes' style='text-align:center;'>".Likes(
        Hsc($cap),
        "",
        "http://".$RealHost.$path,
...
function GetLentaList()
{
    ...
    $where = $GLOBAL["USER"]["role"] == 0 ? "WHERE (`".$table."`.`domain`='".$SubDomain."' AND `".$table."`.`stat`=1)" : "WHERE (`".$table."`.`domain`='".$SubDomain."')";
    $onpage = $node["onpage"];
    $pg = $dir[2] ? $dir[2] : 1;
    ...
    $data = DB(
      "SELECT `".$table."`.id,  `".$table."`.name, `".$table."`.uid, `".$table."`.pic, `".$table."`.data,`".$table."`.comcount, `".$table."`.comments, `".$table."`.`voting`, `".$table."`.`votingend`, `".$table."`.`votingmode`, `".$table."`.`winnerscount`, `".$table."`.`domain` FROM `".$table."` WHERE (`".$table."`.`stat`=1) GROUP BY 1 ".$orderby." LIMIT $from, $onpage"
    );
    ...
    if ($data["total"] > 0) {
        for ($i = 0; $i < $data["total"]; $i++) {
            ...
            $data2 = DB(
              "SELECT `".$table2."`.*, COUNT(`".$table3."`.`id`) as `cnt` FROM `".$table2."` LEFT JOIN `".$table3."` ON `".$table3."`.`vid`=`".$table2."`.`id` WHERE (`".$table2."`.`link`='".$link."' AND `".$table2."`.`pid`=".$item["id"]." AND `".$table2."`.`stat`=1) GROUP BY 1 ORDER BY `".$table2."`.`rate`"
            );
            $pics = '<table><tr>';
            ...
            if ($GLOBAL["now"] < $item["votingend"] && $item['voting'] == 1 && !$_SESSION['userid']) {
                ...
                if ($GLOBAL["now"] < $item["votingend"] && $q["total"]) {
                    $script = '<script type="text/javascript">$("#node'.$item['id'].'.votingCon .votingButton a").remove(); $("#node'.$item['id'].'.votingCon .Info").html("Ваш голос учтён").show();</script>';
                }
            }
            $text .= $script;
    ...
    $data = DB("SELECT count(id) as `cnt` FROM `".$table."` $where");
    @mysql_data_seek($data["result"], 0);
    $ar = @mysql_fetch_array($data["result"]);
    $text .= Pager2(
      $pg,
