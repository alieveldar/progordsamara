...
$data = DB(
  "SELECT `sets` FROM `_pages` WHERE (`module`='strochki' && `stat`='1') LIMIT 1"
);
if ($data["total"] != 1) {
...
function GetIndex()
{
    ...
    if ((int)$_SESSION["str_userid"] == 0) {
        ...
        if (isset($_SESSION['Data']["loginbutton"])) {
            $data = DB(
              "SELECT `id` FROM `".$table3."` WHERE (`login`='".str_replace(
                array(
                  "select",
                  "in",
                  "or",
        ...
        if (isset($_SESSION['Data']["registerbutton"])) {
            ...
            if (!Email_check($P["login2"])) {
                ...
                if ($er == 0) {
                    ...
                    $data = DB(
                      "INSERT INTO `".$table3."` (`login`,`pass`,`name`) VALUES ('".str_replace(
                        array(
                          "select",
                          "in",
                          "or",
    ...
    if (isset($_SESSION['Data']["sendbutton"])) {
        ...
        if ($P["name"] == "" || (int)$P["price"] == 0) {
            ...
            if ((int)$_SESSION['OrderId'] == 0) {
                DB(
                  "INSERT INTO `".$table4."` (`price`,`fio`,`text`,`data`) values ('".(int)$P["price"]."','".str_replace(
                    array(
                      "select",
                      "in",
                      "or",
                    ...
                    "",
                    htmlspecialchars(strip_tags($P["name"]))
                  )."','".str_replace(
                    array(
                      "select",
                      "in",
                    ...
                    "",
                    htmlspecialchars(strip_tags($P["textarea"]))
                  )."','".time()."')"
                );
            ...
            $OrderId = $_SESSION['OrderId'];
            $signature = md5(
              $SETS[0].":".(int)$P["price"].":".(int)$OrderId.":".$SETS[1]
            );
			...
			Заказ #<b>".(int)$OrderId."</b><br>ФИО: <b>".htmlspecialchars(
                strip_tags($P["name"])
              )."</b><br>Сумма: <b>".$P["price"]."</b><br>Комментарий: <b>".htmlspecialchars(
                strip_tags($P["textarea"])
              )."</b>".
...
function GetCabinet()
{
    ...
    if ((int)$_SESSION["str_userid"] == 0) {
        ...
        if (isset($_SESSION['Data']["loginbutton"])) {
            $data = DB(
              "SELECT `id` FROM `".$table3."` WHERE (`login`='".$P["login1"]."' && `pass`='".$P["pass1"]."' && `stat`='1') LIMIT 1"
            );
            if ($data["total"] == 1) {
                @mysql_data_seek($data["result"], 0);
                $ar = @mysql_fetch_array($data["result"]);
        ...
        if (isset($_SESSION['Data']["registerbutton"])) {
            ...
            if (!Email_check($P["login2"])) {
                ...
                $data = DB(
                  "SELECT `id` FROM `".$table3."` WHERE (`login`='".$P["login2"]."') LIMIT 1"
                );
                if ($data["total"] != 0) {
                    $er = 1;
                    $_SESSION["Msg2"] = "<div class='ErrorDiv'>Данный логин уже занят! <a href='/".$dir[0]."/forget/'><u>Забыли пароль</u>?</a></div>";
                ...
                if ($er == 0) {
                    $np = rand(11111111, 999999999);
                    $data = DB(
                      "INSERT INTO `".$table3."` (`login`,`pass`,`name`) VALUES ('".$P["login2"]."','".$P["pass2"]."','".$P["name2"]."')"
                    );
                    $_SESSION["Msg2"] = '<div class="SuccessDiv">Успешно! Пароль отправлен вам на Email</div>';
                    $body = "Здравствуйте, ".$P["name2"]."<br><br>Вы зарегистрировались на сайте <a href='http://".$VARS["mdomain"]."'>".
                      $VARS["mdomain"]."</a> в личном кабинете.Теперь вы сможете  размещать  свои объявления в газете \"Pro Город\" Казань и оплачивать их. Для того что бы подать объявление в газету \"Pro Город\" вам нужно перейти в личный кабинет и ввести 
...
для того чтобы вы смогли подавать объявления в будущем.</td></tr>';
        ...
        $data = DB(
          "SELECT * FROM `".$table3."` WHERE (`id`='".(int)$_SESSION["str_userid"]."') LIMIT 1"
        );
        @mysql_data_seek($data["result"], 0);
        $ar = @mysql_fetch_array($data["result"]);
...
function GetNewAdd()
{
    ...
    $prses = array();
    $data = DB(
      "SELECT * FROM `".$dir[0]."_razdels` WHERE (`stat`='1') ORDER BY `rate` DESC"
    );
    ...
    if (isset($Data["regbutton"])) {
        ...
        if ((int)$Data["city"] == 2) {
            $arpr[0] = round($arpr[0] * 0.57);
            $arpr[1] = round($arpr[1] * 0.68);
            $arpr[2] = round($arpr[2] * 0.81);
            $arpr[3] = round($arpr[3] * 0.83);
        }
	...
	'".$sets."', '".time(
          )."', '".$dts."', 'Выходов: ".$exit.". Символов: ".$ll.". По разделу: ".$onznak."p. Скидка: ".(100 - 100 * $Data["hs"])."%. ".$oneprice."p. x ".$exit."шт. - ".(100 - 100 * $Data["hs"])."% = ".$price."p.')";
        $d = DB($q);
        $dbl = DBL();
        DB(
          "INSERT INTO `".$dir[0]."_pays` (`uid`, `oid`, `price`, `data`) VALUES ('".$_SESSION["str_userid"]."', '".$dbl."', '".$price."', '".time(
          )."');"
        );
        SD();
        if ($er == 0) {
    ...
    $n = time();
    if ((date("w") > 3 || date("w") == 0) && date("d") > 24) {
        $n = $n + 7 * 24 * 60 * 60;
    }
    $t1 = $n;
    $t2 = $n + 1 * 31 * 24 * 60 * 60;
    ...
    $text .= "<table><tr><td width='33%' style='padding:5px;' valign='top'>".CreateCal(
        $t1
      )."</td><td width='34%' style='padding:5px;' valign='top'>".CreateCal(
        $t2
      )."</td><td width='33%' style='padding:5px;' valign='top'>".CreateCal(
        $t3
      )."</td></tr></table>"
      .$C15."<table><tr><td width='33%' style='padding:5px;' valign='top'>".CreateCal(
        $t4
      )."</td><td width='34%' style='padding:5px;' valign='top'>".CreateCal(
        $t5
      )."</td><td width='33%' style='padding:5px;' valign='top'>".CreateCal(
...
function GetMyObjs()
{
    ...
    for ($i = 0; $i < $data["total"]; $i++): @mysql_data_seek(
      ...
      $i
        ...
        if ($ar["statpay"] == 1) {
            ...
            $signature = md5(
              $SETS[0].":".(int)$ar["price"].":".(int)$ar["idpay"].":".$SETS[1]
            );
	...
	<td><b>Город:</b> '.$city.'<div class="C5"></div><b>Текст:</b> '.$ar["text"].'<div class="C5"></div><b>Телефон:</b> '.$ar["phone"].'<div class="C5"></div><b>Дополнительно:</b><br>Выделение цветом:'.$d1.'<br>Выделено рамкой:'.$d2.'<br>Выделено жирным шрифтом:'.$d3.'<br>Выделено ЗАГЛАВНЫМИ БУКВАМИ: '.$d4.'<div class="C5"></div><span style="color:#777; font-size:10px;"><b>Дата выхода:</b> '.str_replace(
            ",",
            ", ",
            $ar["datas"]
          ).'</span></td>
...
function GetSetts()
{
    ...
    if (isset($Data["regbutton"])) {
        DB(
          "UPDATE `".$dir[0]."_users` SET `name`='".$Data["name"]."', `phone`='".$Data["phone"]."' WHERE (`id`='".$_SESSION["str_userid"]."')"
        );
        if ($Data["pass"] != "") {
            DB(
              "UPDATE `".$dir[0]."_users` SET `pass`='".$Data["pass"]."' WHERE (`id`='".$_SESSION["str_userid"]."')"
            );
        }
    ...
    $data = DB(
      "SELECT `name`, `phone`, `login` FROM `".$table3."` WHERE (`id`='".$_SESSION["str_userid"]."')"
    );
    if ($data["total"] == 1) {
        @mysql_data_seek($data["result"], 0);
        $ar = @mysql_fetch_array($data["result"]);
