...
function GetIndex()
{
    global $SETS, $VARS, $GLOBAL, $node, $link, $C15, $C10, $C, $C5;
    $cap = $node["name"];
...
function GetNewAdd()
{
    ...
    if (isset($Data["regbutton"])) {
        ...
        if ((int)$Data["city"] == 2) {
            $arpr[0] = round($arpr[0] * 0.387);
            $arpr[1] = round($arpr[1] * 0.475);
            $arpr[2] = round($arpr[2] * 0.589);
            $arpr[3] = round($arpr[3] * 0.569);
        }
        $price = $arpr[3];
        if ($ll < 151) {
            $price = $arpr[3];
        ...
        $data = DB(
          "INSERT INTO `".$table3."` (`login`,`pass`,`name`,`phone`) VALUES ('auto','".time(
          )."','".$Data["usname"]."','".$Data["usphone"]."')"
        );
        $LASTUSER = DBL();

        $q = "INSERT INTO `".$dir[0]."_objects` (`uid`, `city`, `text`, `phone`, `rid`, `price`, `stat`, `sets`, `data`, `datas`, `dop`) VALUES ('".$LASTUSER."', '".$Data["city"]."', '".$Data["obj"]."', '".$Data["phone"]."', '".(int)$Data["cat"]."', '$price', '0','".$sets."', '".time(
          )."', '".$dts."', 'Выходов: ".$exit.". Символов: ".$ll.". По разделу: ".$onznak."p. Скидка: ".(100 - 100 * $Data["hs"])."%. ".$oneprice."p. x ".$exit."шт. - ".(100 - 100 * $Data["hs"])."% = ".$price."p.')";
        $d = DB($q);
        $dbl = DBL();

        DB(
          "INSERT INTO `".$dir[0]."_pays` (`uid`, `oid`, `price`, `data`) VALUES ('".$LASTUSER."', '".$dbl."', '".$price."', '".time(
          )."');"
        );
        $dpay = DBL();
