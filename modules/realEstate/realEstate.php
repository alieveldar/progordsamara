    ...
    case 'view' :
        ...
        if ($realEstateItem == null) {
            ...
            if ($realEstateItem['spec'] == 1) {
                $text .= displayRealEstateMainContentItem(
                  $realEstateItem,
                  $dir[0],
                  $forums
                );
                $Page["TopContent"] = displayRealEstateTopContentItem(
                  $realEstateItem,
                  $dir[0],
                  $forums
                );
                $contacts = getRealEstateContactsById($itemId);
                $Page["RightContent"] = displayRealEstateRightContentItem(
                    $realEstateItem,
                    $dir[0],
                    $contacts
                  ).$Page["RightContent"];
                $Page["LeftContent"] = displayRealEstateLeftContentItem(
                    $realEstateItem,
                    $dir[0],
                    $contacts
                  ).$Page["LeftContent"];
                ...
                $Page["TopContent"] = displayRealEstateTopContentItemSingle(
                  $realEstateItem
                );
            }
...
function getRealEstateMap()
{
    ...
    for ($i = 0; $i < $data["total"]; $i++) {
        ...
        $events .= '['.$ar2['pid'].', "'.htmlspecialchars(
            $ar2['name']
          ).'", "'.$ar2['maps'].'"';
        $events .= ',"'.$link.'"';
        $events .= ',"'.(int)$ar2["spec"].'"';
        ...
        if ($ar2['data']) {
            $dates .= $dates != '' ? ','.date('d-n-Y', $ar2['data']) : date(
              'd-n-Y',
              $ar2['data']
            );
        }
...
function displayRealEstateMainContentItem(
  $realEstateItem,
  $baseUrl,
  array $forums = null
) {
...
function displayRealEstateLeftContentItem(
  $realEstateItem,
  $baseUrl,
  array $forums = null
) {
...
function displayRealEstateRightContentItem(
  $realEstateItem,
  $baseUrl,
  $contacts = null
) {
