...
if ($HTTPREFERER == $_SERVER['SERVER_NAME']) {
    ...
    $event = @mysql_fetch_array($data["result"]);
    $text = '';
    if ($event['pic']) {
        $text .= '<div class="eventPic"><img src="/userfiles/picnews/'.$event['pic'].'"></div>';
    }
    $text .= $event['text'];
    if ($event['pid'] && $R["readmore"]) {
        $text .= $C10.'<a href="/'.$event['link'].'/view/'.$event['pid'].'" class="readmore">Читать подробнее</a>';
    }
