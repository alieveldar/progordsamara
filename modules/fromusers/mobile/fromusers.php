...
if (!$start) {
    $text .= '<div class="TRLine">Категория<div><select name="cat">'.GetSelected(
        $cats,
        $cat['id']
      ).'</select></div></div>';
}
if ($UserSetsSite[5] == '1') {
    $text .= '<div class="TRLine">Код с картинки<star>*</star><div><img src="/modules/standart/captcha/Captcha.php?'.time(
      ).'" class="captchaImg" /><input name="captcha" type="text" class="captchaInput"></div></div>';
}
