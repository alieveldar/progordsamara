...
$file = $table."2-".$start.".".$page.".".$id;
$dir[0] = "ls";
$node["onpage"] = 33;
$node["orderby"] = "ORDER BY `data` DESC";

// BAN-30-2 -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
$Page["BottomContent"] = "<div class='bannerbox2'><div class='banner' id='Banner-30-2'></div></div>".$C;

if ($link == "ls" && (int)$page == 2216) {
    @header("location: /afisha2504");
    exit();
}
if ($link == "ls" && (int)$page == 2215) {
    @header("location: /detoks");
    exit();
}
if ($link == "ls" && (int)$page == 2231) {
    @header("location: /afisha_may_1_9");
    exit();
}
...
$Page["Caption"] = $cap;
$ShowCap = 1;
if ($link == "ls" && (int)$dir[2] == 33) {
    $node["specdesign"] = "shite";
}
...
if ($start == "view") {
    ...
    $data = DB(
      "SELECT `id`,`comments`, `promo`,`alttext`,`name`,`cat` FROM `".$table."` WHERE (`id`='".(int)$dir[2]."' ".$where.") LIMIT 1"
    );
    if ($data["total"] == 1) {
...
//--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- 		
            ...
            ### Читайте также
            $text .= $C15.'<h2 class="Cat7">Самое новое</h2>';
            $p = DB(
              "SELECT `id`,`name`,`pic` FROM `".$table."` WHERE (`stat`=1 && `id`!='".(int)$dir[2]."') ORDER BY `data` DESC LIMIT 6"
            );
            $text .= "<div>";
...
function GetLentaList()
{
	...
	LEFT JOIN `".$dir[0]."_cats` ON `".$dir[0]."_cats`.`id`=`".$table."`.`cat` WHERE (`".$table."`.`cat`='".(int)$dir[2]."' && `".$table."`.`stat`=1) GROUP BY 1 ".$orderby." LIMIT $from, $onpage";
    $data = DB($q);
    for ($i = 0; $i < $data["total"]; $i++) {
        ...
        if ($ar["pic"] != "") {
            ...
            if (!is_file('userfiles/'.$path.'/'.$ar["pic"])) {
                $path = "picitem";
            }
            $pic = "<img src='/userfiles/".$path."/".$ar["pic"]."' title='".$ar["name"]."' />";
        } else {
            $pic = "";
        }
        ...
        if (($i + 1) % 5 == 0) {
            $text .= "<clear></clear></div><div class='banner' id='Banner-31-".$ban31."'></div>".$C25.$C20."<div>";
            $ban31++;
        }
    }
    ...
    mysql_data_seek($data["result"], 0);
    $ap = @mysql_fetch_array($data["result"]);
    $text .= Pager2(
      $pg,
      $onpage,
      ceil($ap["cnt"] / $onpage),
...
function GetRelevantNews($art, $limit, $tags2)
{
    ...
    $qr = "SELECT `pic`,`data`,`name`,`id` FROM `".$tab."` WHERE ((".trim(
        $q,
        "OR "
      ).") AND (`id`!='".(int)$dir[2]."') AND (`stat`='1')) ORDER BY `data` DESC LIMIT ".$limit;
    $data2 = DB($qr);
    if ($data2["total"] > 0) {
        for ($i = 0; $i < $data2["total"]; $i++): @mysql_data_seek(
          $data2["result"],
          $i
        );
...
function GetLentaId()
{
    ...
    if ($item["pic"] != "") {
        ...
        if ($item["cens"] != "") {
            $text .= "<div class='Cens'>".$item["cens"]."</div>";
        }
        if ($item["picauth"] != "") {
            $text .= "<div class='PicAuth'>Фото: ".$item["picauth"]."</div>";
        }
        $text .= "</div>";
    }

    if ($item["lid"] != "") {
        $text .= $C20."<div class='ItemLid'>".$item["lid"]."</div>";
    }
    if ($item["promo"] != 1 && (int)$item["id"] != 1579 && (int)$item["id"] != 1633) {
        $text .= $C20."<div class='banner' id='Banner-31-1'></div>";
    }
    if ($item["text"] != "") {
        $text .= $C20.$item["text"];
    }
    ...
    if ($p["total"] > 0) {
        $text .= $C20."<h3>Фотоальбом:</h3>$C10<div class='ItemAlbum'>";
        for ($i = 0; $i < $p["total"]; $i++): mysql_data_seek($p["result"], $i);
            $ar = @mysql_fetch_array($p["result"]);
            $text .= "<a href='/userfiles/picoriginal/".$ar["pic"]."' title='".$ar["name"]."' rel='prettyPhoto[gallery]'><img src='/userfiles/pictavto/".$ar["pic"]."' title='".$ar["name"]."' alt='".$ar["name"]."'></a>"; endfor;
        $text .= "</div>".$C;
    }

    ### Голосование
    if ((int)$item["vvid"] != 0) {
        $text .= $C20."<div id='ItemVotingDiv'></div><script>GetItemVoting(".(int)$item["vvid"].");</script>".$C5;
    }
    ...
    if ($item["endtext"] != "") {
        $text .= $C20."<div class='TextQuot'>".$item["endtext"]."</div>".$C10;
    }
    ...
    ### Лайки
    $text .= $C10."<div class='Likes' style='text-align:center;'>".Likes(
        Hsc($cap),
        "",
        "http://".$RealHost.$path,
        Hsc(strip_tags($lid))
    ...
    ### Читайте также
    $text .= '<h2 class="Cat7">Читайте также:</h2>';
    $p = DB(
      "SELECT `id`,`name`,`pic` FROM `".$table."` WHERE (`stat`=1 && `cat`='".$item["cat"]."' && `id`!='".(int)$dir[2]."') ORDER BY `data` DESC LIMIT 3"
    );
    $text .= "<div>";
    for ($i = 0; $i < $p["total"]; $i++): mysql_data_seek($p["result"], $i);
        $ar = @mysql_fetch_array($p["result"]);
        $text .= "<div class='sem3td rm'><a href='/".$link."/view/".$ar["id"]."' title='$ar[name]'><img src='/userfiles/pictavto/$ar[pic]'>$ar[name]</a></div>"; endfor;
    $text .= "</div>";
    $text .= $C10;
    ...
    $mixblock .= "<div class='ItemTags'>".$tags."</div>".$C10;
    ...
    if ($item["uid"] != 0 && $item["nick"] != "") {
        $auth = $avatar."Автор: <a href='http://".$VARS["mdomain"]."/users/view/".$item["uid"]."/'>".$item["nick"]."</a>, ".$d[1];
    } else {
    ...
    ### Платные ссылки
    if ($item["adv"] != "") {
        $text .= $C10."<div class='CBG'></div>".$C5."<div class='AdvBlock'>".$item["adv"]."</div>".$C;
    }
