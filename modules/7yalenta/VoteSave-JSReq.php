...
if ($HTTPREFERER == $_SERVER['SERVER_NAME']) {
    ...
    $q = DB(
      "SELECT `id` FROM `$table2` WHERE (".$or.") AND `pid`=$pid AND `link`='$link')"
    );
    ...
    if ($q["total"] == 1) {
        ...
        DB(
          "INSERT INTO `$table2` (`link`,`vid`,`uid`,`data`,`ip`,`pid`) VALUES ('$link', $vid, ".$_SESSION['userid'].", '".$GLOBAL["now"]."', '".$GLOBAL["ip"]."', '$pid')"
        );
        ClearCache($file);
