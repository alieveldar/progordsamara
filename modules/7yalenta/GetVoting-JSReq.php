...
if ($HTTPREFERER == $_SERVER['SERVER_NAME']) {
    ...
    function getVotingForm()
    {
        ...
        if (RetCache($file) == "true") {
            ...
            $data = DB(
              "SELECT `$table`.*, COUNT(`$table2`.`id`) as `vn` FROM `$table` LEFT JOIN `$table2` ON `$table2`.`vid`=`$table`.`id` AND `$table2`.`pid`=`$table`.`pid` AND `$table2`.`link`=`$table`.`link` WHERE (`$table`.`id`=$qid OR `$table`.`vid`=$qid) GROUP BY 1"
            );
            if ($data["total"] >= 3) {
                $form = '<form action="">';
                for ($i = 0; $i < $data["total"]; $i++) {
                    ...
                    if ($ar['vid'] == 0) {
                        ...
                        $form .= '<div><input type="radio" name="vote" value="'.$ar["id"].'"/> '.$ar["name"].'</div>';
                    }
                }
            ...
            if ($data["total"] >= 3) {
                ...
                for ($i = 0; $i < $data["total"]; $i++) {
                    ...
                    if ($ar['vid'] == 0) {
                        ...
                        if ($last_symbol == 0 || $last_symbol > 4 || (substr(
                              $ar["vn"],
                              -2
                            ) > 10 && substr($ar["vn"], -2) < 20)) {
                            $index = 0;
                        } else {
                            if ($last_symbol == 1) {
                                $index = 1;
                        ...
                        $res .= '<div class="votingResult"><span class="votingResultLine" style="width:'.($ar["vn"] * $k + 10).'px">'.round(
                            $ar["vn"] / $total * 100
                          ).'%</span>'.$ar["name"].' <span class="votingResultNumber">('.$ar["vn"].' '.$votesWords[$index].')</span></div>';
                    }
                }
    ...
    $data = DB(
      "SELECT `$table`.*, COUNT(`$table2`.`id`) as `vn` FROM `$table` JOIN `$table2` ON `$table2`.`vid`=`$table`.`id` AND `$table2`.`pid`=`$table`.`pid` AND `$table2`.`link`=`$table`.`link` AND ($or) WHERE (`$table`.`vid`=$qid) GROUP BY 1"
    );
    if ($data["total"]) {
        $result['text'] = getVotingResult();
    } else {
        $result['text'] = getVotingForm();
    }
}
