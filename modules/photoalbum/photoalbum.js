$(document).ready(function () {
    var Current, i;
    var img_count = $('.Carusel .Slider a').size();
    var img_max = img_count > 9 ? 9 : img_count;
    $('.Carusel .Slider a').each(function (index) {
        if ($(this).attr('class') == 'current') Current = index;
    });
    if (img_max < 5) $('.Carusel .Container').css({
        'width': ($('.Carusel .Slider img:first').width() + 6) * img_max,
        'margin': '0 auto'
    });
    else $('.Carusel .Slider img').width($('.Carusel .Container').width() / img_max - 6);
    if (Current < Math.floor(img_max / 2)) {
        for (i = 0; i < Math.floor(img_max / 2) - Current; i++) $('.Carusel .Slider a:eq(' + (img_count - 1) + ')').prependTo($('.Carusel .Slider'));
    }
    else if (Current > Math.floor(img_max / 2)) {
        for (i = 0; i < Current - Math.floor(img_max / 2); i++) $('.Carusel .Slider a:eq(0)').appendTo($('.Carusel .Slider'));
    }
...
function SetMapPoint(id, balloonId) {
    ...
    if ($('.pic_' + id + '').text()) {
        ...
        marker.setIcon(new DG.Icon('/userfiles/picnews/' + $('.pic_' + id + '').text(), new DG.Size(35, 35), function () {
            return new DG.Point(-7, -51)
        }));
    }
}

function ItemDelete(id, link, act) {
    ActionAndUpdate(id, act, link);
}

function ActionAndUpdate(id, act, link) {
    JsHttpRequest.query('/modules/photoalbum/photoalbum-JSReq.php', {
        'id': id,
        'act': act,
        'link': link
    }, function (result, errors) {
        if (result) {
            if (act == "DELPHOTO" || act == "DELALBUM") {
                $("#Act" + id).parents('.Item').remove();
            }
        }
    }, true);
}
