...
if ($HTTPREFERER == $_SERVER['SERVER_NAME']) {
    ...
    if ($R["act"] == "DELPHOTO") {
        $data = DB(
          "SELECT `".$table2."`.`pic`, `".$table2."`.`uid`, `".$table."`.`uid` AS `puid` FROM `".$table2."` LEFT JOIN `".$table."` ON `".$table."`.`id`=`".$table2."`.`pid` WHERE (`".$table2."`.`id`=".$id.") limit 1"
        );
    ...
    if ($R["act"] == "DELALBUM") {
        $data = DB(
          "SELECT `uid` FROM `".$table."` WHERE (`id`=".$id.") limit 1"
        );
        @mysql_data_seek($data["result"], 0);
        $item = @mysql_fetch_array($data["result"]);
        if ($item['uid'] == $_SESSION['userid'] || $_SESSION['userrole'] > 2) {
            ...
            for ($i = 0; $i < $data["total"]; $i++) {
                ...
                foreach ($GLOBAL['AutoPicPaths'] as $path => $size) {
                    @unlink($ROOT."/userfiles/".$path."/".$ar2['pic']);
                }
            }
        }
    }
