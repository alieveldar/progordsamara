...
if ($start == "albums") {
    $data = DB(
      "SELECT `name`, `sets`, `text` FROM `_pages` WHERE (`link`='".$link."' && `stat`='1') limit 1"
    );
    if (!$data["total"]) {
        ...
        if ($sets[2]) {
            if ($_SESSION['userid']) {
                $text .= '<script type="text/javascript">$(".Add").css({"float":"right", "margin-top":-35}).html(\'<a href="/'.$dir[0].'/add" class="SaveButton">Добавить свой альбом</a>\').after(\'<div class="C10"></div>\');</script>';
            } else {
                $redirect_uri = rawurlencode(
                  "http://".$RealHost."/modules/standart/LoginSocial.php?back=http://".$RealHost."/".$dir[0]."/addalbum"
                );
...
if ($start == "view") {
    $data = DB(
      "SELECT `".$table."`.*, `_users`.`nick`, `_users`.`avatar`, (SELECT `".$table2."`.`pic` FROM `".$table2."` WHERE (`".$table2."`.`pid`=`".$table."`.`id`) ORDER BY `".$table2."`.`rate` ASC LIMIT 1) AS `photo1` FROM `".$table."` LEFT JOIN `_users` ON `_users`.`id`=`".$table."`.`uid` WHERE (`".$table."`.`id`=".$dir[2]." && `".$table."`.`stat`='1') limit 1"
    );
    if (!$data["total"]) {
        $cap = "Материал не найден";
        $text = @file_get_contents($ROOT."/template/404.html");
        ...
        if ($item['photofromusers'] && $_SESSION['userid'] != $item['uid'] && $GLOBAL['USER']['role'] < 3) {
            if ($_SESSION['userid']) {
                $text .= '<script type="text/javascript">if(!$(".winner").size()) { $(".Add").html(\'<a href="/'.$dir[0].'/addphoto/'.$item['id'].'" class="SaveButton">Добавить своё фото</a>\').after(\'<div class="C15"></div>\'); } jQuery.each($(".Item"), function(i, val) { var fid=$(this).attr("id"); id=fid.split("-"); if(id[2] == '.$_SESSION['userid'].') $(this).append("<div class=\'AdminPanel\'><div id=\'Act"+id[1]+"\' class=\'Act\'><a href=\'javascript:void(0);\' onclick=\'ItemDelete("+id[1]+", \"'.$dir[0].'\", \"DELPHOTO\")\' title=\'Удалить фотографию\'><img src=\'/template/standart/exit.png\'></a></div><div class=\'Act\'><a href=\'/'.$dir[0].'/editphoto/"+id[1]+"\' title=\'Настройки фотографии\'><img src=\'/template/standart/edit.png\'></a></div></div>"); });</script>';
            } else {
                $redirect_uri = rawurlencode(
                  "http://".$RealHost."/modules/standart/LoginSocial.php?back=http://".$RealHost."/".$dir[0]."/addphoto/".$item['id']
                );
            ...
            if ($_SESSION['userid'] == $item['uid'] || $GLOBAL['USER']['role'] > 2) {
...
if ($start == "edit") {
    $data = DB(
      "SELECT `".$table."`.*, `_pages`.`sets`, `_users`.`nick`, `_users`.`avatar`, (SELECT `".$table2."`.`pic` FROM `".$table2."` WHERE (`".$table2."`.`pid`=`".$table."`.`id`) ORDER BY `".$table2."`.`rate` ASC LIMIT 1) AS `photo1` FROM `".$table."` LEFT JOIN `_users` ON `_users`.`id`=`".$table."`.`uid` LEFT JOIN `_pages` ON `_pages`.`link`='".$link."' WHERE (`".$table."`.`id`=".$dir[2]." && `".$table."`.`uid`=".$_SESSION['userid'].") limit 1"
    );
    if (!$data["total"]) {
        $cap = "Материал не найден";
        $text = @file_get_contents($ROOT."/template/404.html");
        ...
        @mysql_data_seek($data["result"], 0);
        $item = @mysql_fetch_array($data["result"]);
        if ($_SESSION['userid'] == $item['uid']) {
            list($text, $cap) = EditAlbum($item);
        }
    }
}
...
if ($start == "add") {
    ...
    if (!$data["total"]) {
        ...
        $ar = @mysql_fetch_array($data["result"]);
        $sets = explode('|', $ar['sets']);
        if ($sets[2] && $_SESSION['userid']) {
            list($text, $cap) = AddAlbum($sets);
        }
    }
}
...
if ($start == "addphoto") {
    $data = DB(
      "SELECT `".$table."`.* FROM `".$table."` WHERE (`".$table."`.`id`=".$dir[2]." && (`".$table."`.`stat`='1' OR `".$table."`.`uid`=".$_SESSION['userid'].")) limit 1"
    );
    if (!$data["total"]) {
        $cap = "Материал не найден";
        $text = @file_get_contents($ROOT."/template/404.html");
        ...
        @mysql_data_seek($data["result"], 0);
        $item = @mysql_fetch_array($data["result"]);
        if (($item['photofromusers'] && $_SESSION['userid']) || $_SESSION['userid'] == $item['uid']) {
            list($text, $cap) = AddPhoto($item);
        }
    }
}
...
if ($start == "editphoto") {
    $data = DB(
      "SELECT `".$table2."`.*, `".$table."`.`email`, `".$table."`.`name` AS `pname`, `".$table."`.`uid` AS `puid`, `".$table."`.`photoapproval` FROM `".$table2."` LEFT JOIN `".$table."` ON `".$table."`.`id`=`".$table2."`.`pid` WHERE (`".$table2."`.`id`=".$dir[2]." && `".$table2."`.`uid`=".$_SESSION['userid'].") limit 1"
    );
    if (!$data["total"]) {
        $cap = "Материал не найден";
        $text = @file_get_contents($ROOT."/template/404.html");
        ...
        $item = @mysql_fetch_array($data["result"]);
        if (($item['photofromusers'] && $_SESSION['userid']) || $_SESSION['userid'] == $item['uid']) {
            list($text, $cap) = EditPhoto($item);
        }
    }
}

$Page["Content"] = $edit.$text;
$Page["Caption"] = $cap;
...
function GetAlbums()
{
    ...
    $from = ($pg - 1) * $onpage;
    $data = DB(
      "SELECT `".$table."`.`id`, `".$table."`.`name`, `".$table."`.`uid`, `".$table."`.`data`, `".$table."`.`concurs`, (SELECT `".$table2."`.`pic` FROM `".$table2."` WHERE (`".$table2."`.`pid`=`".$table."`.`id`) ORDER BY `".$table2."`.`rate` ASC LIMIT 1) AS `photo1`, `".$table2."`.`pic`, `_users`.`nick`
    ...
    if ($data["total"]) {
        ...
        for ($i = 0; $i < $data["total"]; $i++) {
            ...
            if ($ar2['concurs']) {
                $text .= '<img src="/template/standart/concurs-tag.png" class="tag">';
            }
            $text .= '</ins>';
        }
        $text .= '</div>';
...
function GetAlbum($item)
{
    ...
    $text = "";
    $path = '/userfiles/picnews/'.($item['pic'] ? $item['pic'] : $item['photo1']);
    $lid = CutText($item["text"], 100);
    ...
    $from = ($pg - 1) * $onpage;
    $data = DB(
      "SELECT * FROM `".$table2."` WHERE (`pid`=".$dir[2]." AND`stat`=1)  GROUP BY 1 ORDER BY `rate` ASC LIMIT $from, $onpage"
    );
    ...
    if ($data["total"]) {
        ...
        for ($i = 0; $i < $data["total"]; $i++) {
            ...
            if ($ar2['winner']) {
                $text .= '<div class="winner">Победитель</div>';
            }
            $text .= '</ins>';
        }
        $text .= '</div>';
        $text .= $C."<div class='Likes'>".Likes(
            Hsc($cap),
            "",
            "http://".$RealHost.$path,
            Hsc(strip_tags($lid))
        ...
        if ($t != "") {
            $ta = DB(
              "SELECT * FROM `".$dir[0]."_tags` WHERE (`id` IN (".$t.")) LIMIT 3"
            );
            for ($i = 0; $i < $ta["total"]; $i++): @mysql_data_seek(
              ...
              $i
                ...
                $tags .= "<a href='/$dir[0]/tags/$ar[id]'>$ar[name]</a>, "; endfor;
            $tags2 = trim($tags, ", ");
            $tags = "Тэги:".trim($tags, ", ");
        }
...
function GetPhoto($item)
{
    ...
    $orderby = $ORDERS[$node["orderby"]];
    $text = "";
    $cap = $item["name"] ? $item["name"] : 'Фотография без названия';
    $d = ToRusData($item["data"]);
    ...
    if ($item['author']) {
        if (!$item['concurs']) {
            $text .= '<div class="PicAuth">Автор фотографии: '.$item['author'].'</div>';
        } else {
            $text .= '<div class="PicAuth"><h3 class="CenterText">Автор фотографии: '.$item['author'].'</h3></div>';
        }
    }
    if ($item["text"]) {
        $text .= $C10.$item["text"];
    ...
    $text .= $C15."<div class='Likes'>".Likes(
        Hsc($cap),
        "",
        "http://".$RealHost.$path,
        Hsc(strip_tags($lid))
    ...
    if ($item["pavatar"] == "" || !is_file(
        $ROOT."/".$item["pavatar"]
      ) || filesize($ROOT."/".$item["pavatar"]) < 100) {
        $avatar = "<img src='/userfiles/avatar/no_photo.jpg'>";
        if (in_array(
          $RealHost,
          array('nsk.progorodsamara.ru', 'nsk.samara.cc')
        ...
        $avatar = "<img src='/".$item["pavatar"]."'>";
    }
    if (!$item['concurs']) {
        $text .= '<div class="ItemAuth">'.$avatar.'Автор альбома: <a href="/users/view/'.$item['puid'].'">'.$item['pnick'].'</a></div>';
    }
    $t = trim($item["tags"], ",");
    $tags = "";
    if ($t != "") {
    ...
    $data = DB(
      "SELECT `id`, `name`, `pic`, `maps` FROM `".$table2."` WHERE (`pid`=".$item["pid"]." AND`stat`=1)  GROUP BY 1 ORDER BY `rate` ASC"
    );
    if ($data["total"] > 1) {
        ...
        $text .= '</div></div>';
        if ($data["total"] > 9) {
            $text .= '<a href="javascript:void(0);" class="btn l-btn"><img src="/template/standart/tleft1.png" onclick="$(\'.Carusel .Slider a:last\').prependTo($(\'.Carusel .Slider\'));"></a><a href="javascript:void(0);" class="btn r-btn"><img src="/template/standart/tright1.png" onclick="$(\'.Carusel .Slider a:first\').appendTo($(\'.Carusel .Slider\'));"></a>';
        }
    }
...
function AddAlbum($sets)
{
    ...
    if (isset($_SESSION['Data']["SaveButton"])) {
        ...
        if (trim($P['name']) == '') {
            ...
            $stat = $sets[3] ? 0 : 1;
            $q = "INSERT INTO `$table` (`data`, `name`, `text`, `uid`, `stat`, `comments`) VALUES ('".time(
              )."', '".$name."', '".$text."', ".$_SESSION['userid'].", ".$stat.", ".$P['comments'].")";
            DB($q);
            $last = DBL();
            DB(
              "UPDATE `$table` SET `rate`='".$last."' WHERE  (id='".$last."')"
            );
            unset($P);
            if ($stat) {
                $_SESSION['msg'] = '<div class="SuccessDiv">Альбом добавлен</div>';
            } else {
                $_SESSION['msg'] = '<div class="SuccessDiv">Альбом добавлен, но будет отображен после одобрения администратором</div>';
            }
...
function EditAlbum($item)
{
    ...
    if (isset($_SESSION['Data']["SaveButton"])) {
        ...
        if (trim($P['name']) == '') {
            ...
            DB($q);
            if ($stat) {
                $msg = '<div class="SuccessDiv">Альбом отредактирован</div>';
            } else {
                $msg = '<div class="SuccessDiv">Альбом отредактирован, но будет отображен после одобрения администратором</div>';
            }
        }
        SD();
    }
...
function AddPhoto($item)
{
    ...
    if (isset($_SESSION['Data']["SaveButton"])) {
        ...
        if (trim($P['name']) == '' || !$P['pic']) {
            ...
            $q = "INSERT INTO `$table2` (`data`, `name`, `text`, `author`, `pic`, `pid`, `uid`, `maps`, `stat`) VALUES ('".time(
              )."', '".$name."', '".$text."', '".$author."', '".$pic."', ".$item['id'].", ".$_SESSION['userid'].", '".$P['maps']."', ".$stat.")";
            DB($q);
            $last = DBL();
            DB(
              "UPDATE `$table2` SET `rate`='".$last."' WHERE  (id='".$last."')"
            );
            unset($P);
            if ($stat) {
                $msg = '<div class="SuccessDiv">Фотография добавлена</div>';
            } else {
                $msg = '<div class="SuccessDiv">Фотография добавлена, но будет отображена после одобрения администратором</div>';
            }
            if ($item['email']) {
                ...
                if (!$stat) {
                    $body .= "В данное время фотография не опубликована и ожидает вашего одобрения";
                }
    ...
    if ($_SESSION['userid'] == $item['uid']) {
        $text .= "<tr><td></td><td><input name='main' type='checkbox' ".$chk."> Обложка альбома</td></tr>";
    }
    if ($P['pic']) {
        $text .= '<span class="imgCon"><img src="/userfiles/temp/'.$P['pic'].'" class="img" /><img src="/template/standart/exit.png" class="remove" onclick="imgRemove($(this))" /><input type="hidden" name="pic" value="'.$P['pic'].'" /></span>';
    }
    $text .= '</div></td></tr>';
...
function EditPhoto($item)
{
    ...
    if (isset($_SESSION['Data']["SaveButton"])) {
        ...
        if (trim($P['name']) == '' || !$P['pic']) {
            ...
            DB($q);
            if ($stat) {
                $msg = '<div class="SuccessDiv">Фотография отредактирована</div>';
            } else {
                $msg = '<div class="SuccessDiv">Фотография отредактирована, но будет отображена после одобрения администратором</div>';
            }
            if ($item['email']) {
    ...
    $text .= '<tr><td class="VarText" style="vertical-align:top; padding-top:10px;">Фотография</td><td class="LongInput"><div class="uploaderCon" style="'.($P['pic'] ? 'display:none;' : '').'"><div class="uploader"></div><div class="Info">Вы можете загрузить фотографию в формате jpg, gif и png</div></div><div class="uploaderFiles">';
    if ($P['pic']) {
        $text .= '<span class="imgCon"><img src="/userfiles/picpreview/'.$P['pic'].'" class="img" /><img src="/template/standart/exit.png" class="remove" onclick="imgRemove($(this))" /><input type="hidden" name="pic" value="'.$P['pic'].'" /></span>';
    }
    $text .= '</div></td></tr>';
    if ($_SESSION['userid'] == $item['puid']) {
        $text .= "<tr><td></td><td><input name='main' type='checkbox' ".$chk."> Обложка альбома</td></tr>";
    }
