...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
	...
	if ($node["dzenrss"]==1) { $chk11="checked"; }
	
	if ($node["comments"]==0) { $c1="selected"; } elseif ($node["comments"]==1) { $c2="selected"; } else { $c3="selected"; }
	$utags=explode(",", trim($node["tags"], ","));
	
	$data=DB("SELECT `id`, `name` FROM `".$alias."_cats` ORDER BY `rate` DESC"); for ($i=0; $i<$data["total"]; $i++): @mysql_data_seek($data["result"],$i); $ar=@mysql_fetch_array($data["result"]); $site[$ar["id"]]=$ar["name"]; endfor;
	...
	<tr class='TRLine1'>
		<td width='1%'><input name='dzenrss' id='dzenrss' type='checkbox' value='1' $chk11></td><td width='20%'>Отправить в Яндекс.Дзен</td>
	</tr>
	</table></div>";
