...
if ($GLOBAL["sitekey"]==1 && $GLOBAL["database"]==1) {
	...
	$q="SELECT `".$table."`.`id`, `".$table."`.`uid`, `".$table."`.`link`, `".$table."`.`uname` as `uname`, `".$table."`.`pid`, `".$table."`.`text`, `".$table."`.`ip`, `".$table."`.`data`, `".$table2."`.`nick`, `_pages`.`shortname` as `lname`
	FROM `".$table."` LEFT JOIN `".$table2."` ON `".$table2."`.`id`=`".$table."`.`uid` LEFT JOIN `_pages` ON `_pages`.`link`=`".$table."`.`link` ORDER BY `".$table."`.`data` DESC LIMIT $from, $limit";
