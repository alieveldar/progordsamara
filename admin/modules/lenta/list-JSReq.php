...
if ($_SESSION['userrole']>2) {
	...
	if ($R["act"]=="DEL") {
		...
		$itemsea=explode(",", $items);
		
		foreach($itemsea as $item) {
			DB("INSERT INTO `_lentalog` (`link`, `id`, `uid`, `data`, `ip`, `text`) VALUES ('".$link."', '".$item."', '".$_SESSION['userid']."', '".time()."', '".$_SERVER['REMOTE_ADDR']."', 'Удаление публикации (delete)')");
			$result["log"].="INSERT INTO `_lentalog` (`link`, `id`, `uid`, `data`, `ip`, `text`) VALUES ('".$link."', '".$item."', '".$_SESSION['userid']."', '".time()."', '".$_SERVER['REMOTE_ADDR']."', 'Удаление публикации (delete)')";
		}
