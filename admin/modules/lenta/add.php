...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
        ...
        if (isset($P["savebutton"])) {
            ...
            $q = "INSERT INTO `".$alias."_lenta` (`uid`, `bid`, `cat`, `name`, `kw`, `ds`, `cens`, `realinfo`, `comments`, `data`, `astat`, `adata`, `promo`, `spromo`, `onind`, `spec`, `yarss`, `mailrss`, `tavto`, `tags`, `redak`,`gis`,`mailtizer`,`dzenrss`)
		VALUES ('".(int)$P['authid']."', '".(int)$P['bid']."', '".(int)$P["site"]."', '".str_replace(
                "'",
                '&#039;',
                $P["dname"]
              )."', '".str_replace("'", '&#039;', $P["dkw"])."', '".str_replace(
                "'",
                '&#039;',
                $P["dds"]
              )."', '".$P["cens"]."', '".str_replace(
		...
		'".$P["autoon"]."', '".$sdata2."', '".$P["comrs"]."', '".$P["scomrs"]."','".$P["ontv"]."', '".$P["spec"]."', '".$P["yarss"]."', '".$P["mailrss"]."', '".$P["tavto"]."', '".$dtags."', '".$P["redak"]."', '".$P["gis"]."', '".$P["mailtizer"]."','".$P["dzenrss"]."')";

            $_SESSION["Msg"] = "<div class='SuccessDiv'>Новая публикация успешно создана!</div>";
            $data = DB($q);
            $last = DBL();
            DB(
              "UPDATE `".$alias."_lenta` SET `rate`='".$last."' WHERE  (id='".$last."')"
            );
            DB(
              "INSERT INTO `_lentalog` (`link`, `id`, `uid`, `data`, `ip`, `text`) VALUES ('".$alias."', '".$last."', '".$_SESSION['userid']."', '".time(
              )."', '".$_SERVER['REMOTE_ADDR']."', 'Создание #".$last.": ".str_replace(
                "'",
                '&#039;',
                $P["dname"]
              )."')"
...
// ВЫВОД ПОЛЕЙ И ФОРМ
	...
	<tr class="TRLine0"><td class="VarText">Заголовок материала<star>*</star></td><td class="LongInput">	<input name="dname" id="dname" type="text" class="JsVerify2" maxlength="90" style="width:500px; float:left;">
	...
	<tr class="TRLine0 ShowSets"><td class="VarName">Автопубликация</td><td class="DateInput">'.GetDataSet(
            0,
            1
          ).' включить таймер: <input type="checkbox" name="autoon" id="autoon" value="1"></td><tr>
	'."</table></div>";
	...
	<tr class='TRLine0'>
		...
		<td width='1%'><input name='spec' id='spec' type='checkbox' value='1'></td><td width='20%'>Отправить PUSH уведомлением</td>
	</tr>
	<tr class='TRLine1'>
	...
	<tr class='TRLine0'>
		<td width='1%'><input name='dzenrss' id='dzenrss' type='checkbox' value='1'></td><td width='20%'>Отправить в <b>Яндекс.Дзен</b></td>
	</tr>
	</table></div>";
...
<tr class="TRLine1">
		<td><input name="main_pronsk" id="main_pronsk" type="checkbox" value="1"></td><td width="20%">Главная новость <b>ProNSK</b></td>
		<td width="1%"><input name="main_column_pronsk" id="main_column_pronsk" type="checkbox" value="1"></td><td width="20%">Основная колонка <b>ProNSK</b></td>
		<td width="1%"><input name="left_column_pronsk" id="left_column_pronsk" type="checkbox" value="1"></td><td width="20%">Боковая колонка <b>ProNSK</b></td>
		</tr>
		<tr>
		<td width="1%"><input name="main_column_pronsk_both" id="main_column_pronsk_both" type="checkbox" value="1"></td><td width="20%">Отобразить новость <b>в главной колонке</b> в Самаре и Новокуйбышевске</td>
		<td width="1%"><input name="left_column_pronsk_both" id="left_column_pronsk_both" type="checkbox" value="1"></td><td width="20%">Отобразить новость <b>в боковой колонке</b> в Самаре и Новокуйбышевске</td>
		<td width="1%"><input name="commerce_column_pronsk_both" id="commerce_column_pronsk_both" type="checkbox" value="1"></td><td width="20%">Коммерческая новость ProNSK.ru</td>
	</tr>
        </table></div>';
