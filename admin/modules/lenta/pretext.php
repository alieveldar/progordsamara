...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
	...
	if (isset($P["savebutton"])) {
		...
		$q="UPDATE `".$alias."_lenta` SET `pay`='".$P["lid"]."', `alttext`='".str_replace("'","\'",$P["PostText"])."', `endtext`='".$P["Post3Text"]."', `adv`='".$P["Post2Text"]."' WHERE (id='".(int)$id."')"; DB($q);
	...
	if ($data["total"]!=1) { $AdminText=ATextReplace('ItemError', $raz["shortname"]." (".$alias.")", $id); $GLOBAL["error"]=1; } else {
		...
		$AdminText.=$C10."<h2>Коды TILDA (вставьте их все здесь)</h2><textarea name='PostText' style='outline:none; width:100%; height:400px;'>".$node["alttext"]."</textarea>";
