...
if ($GLOBAL["sitekey"]==1 && $GLOBAL["database"]==1) {
	...
	@mysql_data_seek($data["result"], 0); $pag=@mysql_fetch_array($data["result"]);
	$table=$pag["link"]."_razdels"; $table1=$pag["link"]."_objects"; $table2=$pag["link"]."_users"; $table3=$pag["link"]."_pays";
	...
	ORDER BY ".$table1.".`data` DESC  LIMIT $from, $onpage"; $data=DB($q1); $q2="SELECT ".$table1.".id FROM ".$table1; $data2=DB($q2); $pager=Pager($pg, $onpage, ceil($data2["total"]/$onpage)); 
	$AdminText='<h2 style="float:left;">Список объявлений</h2>';
	
	 
	
	for ($i=0; $i<$data["total"]; $i++): @mysql_data_seek($data["result"], $i); $ar=@mysql_fetch_array($data["result"]); $datan=ToRusData($ar["data"]); if ($ar["city"]==1) { $city="Самара"; } else { $city="Новокуйбышевск"; }
	...
	$text.="<td class='BigText' style='padding:8px; text-align:left; color:#333; font-size:11px;'>Текст: ".$ar["text"]."<div class='C5'></div>Контакт: ".$ar["phone"]."<div class='C5'></div><b>".$city."</b> / Раздел: ".$ar["rname"]."</td>";
