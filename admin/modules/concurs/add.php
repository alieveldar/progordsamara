...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
	...
	if (isset($P["savebutton"])) {
		...
		$q="INSERT INTO `".$alias."_lenta` (`uid`, `domain`, `name`, `kw`, `ds`, `cens`, `realinfo`, `comments`, `data`, `astat`, `adata`, `votingend`, `promo`, `onind`, `spec`, `yarss`, `mailrss`, `tavto`, `tags`, `redak`,`gis`, `mailtizer`,`dzenrss`)
		VALUES ('".$_SESSION['userid']."', '".(int)$P["domain"]."', '".$P["dname"]."', '".$P["dkw"]."', '".$P["dds"]."', '".$P["cens"]."', '".$P["realinfo"]."', '".$P["comms"]."', '".$sdata1."',
		'".$P["autoon"]."', '".$sdata2."', '".(time() + 7 * 24 * 60 * 60)."', '".$P["comrs"]."','".$P["ontv"]."', '".$P["spec"]."', '".$P["yarss"]."', '".$P["mailrss"]."', '".$P["tavto"]."', '".$dtags."', '".$P["redak"]."', '".$P["gis"]."', '".$P["mailtizer"]."','".$P["dzenrss"]."')";
...
// ВЫВОД ПОЛЕЙ И ФОРМ
	...
	<tr class='TRLine1'>
		<td width='1%'><input name='dzenrss' id='dzenrss' type='checkbox' value='1'></td><td width='20%'>Отправить в Яндекс.Дзен</td>
	</tr>
	</table></div>";
