...
if ($GLOBAL["sitekey"]==1 && $GLOBAL["database"]==1) {
    ...
    // ЭЛЕМЕНТЫ
    $raz = array('name' => '', 'orderby' => '', 'onpage' => 30);
    ...
    $data=DB("SELECT `".$table."`.*, `".$table2."`.`name` as catn FROM `".$table."` LEFT JOIN `".$table2."` ON `".$table2."`.`id`=`".$table1."`.`cat`
    
    WHERE (main_pronsk = 1 OR main_column_pronsk = 1 OR left_column_pronsk = 1 OR main_column_pronsk_both = 1 OR left_column_pronsk_both = 1)
    ".$orderby." LIMIT $from, $onpage"); $text="";
    ...
    // ПРАВАЯ КОЛОНКА
    $AdminRight="<div class='C20'></div><h2>Категории раздела</h2>"; $text=""; $data=DB("SELECT * FROM `".$table2."` order by `rate` DESC");
